...
(async () => {
    ...
    const report = {
        setup: "Playwright Chromium (Local Dev)",
    ...
    try {
        ...
        console.log("Could not reach local dev server:", e.message);
        report.errors.push("Could not reach local dev server. Is npm run dev running?");
        fs.writeFileSync(path.join(outputDir, 'timetable_report_local.json'), JSON.stringify(report, null, 2));
        await browser.close();
        return;
    }
    ...
    try {
        ...
        await page.waitForTimeout(3000); // Give dashboard time to settle
        ...
        await page.screenshot({ path: path.join(outputDir, 'timetable_local_1440x900.png') });
                ...
                for(let b of classBlocks) {
                    ...
                    if (box && box.width > 20 && box.height > 20) {
                        ...
                        if (dockText) {
                            ...
                            await page.screenshot({ path: path.join(outputDir, 'timetable_local_1440x900_dock.png') });
                        }
                        break;
                    }
                }
        ...
        await page.screenshot({ path: path.join(outputDir, 'timetable_local_900x900.png') });
        ...
        await page.screenshot({ path: path.join(outputDir, 'timetable_local_390x844.png') });
    ...
    fs.writeFileSync(path.join(outputDir, 'timetable_report_local.json'), JSON.stringify(report, null, 2));
    console.log("Done. Report saved to timetable_report_local.json.");
})();
