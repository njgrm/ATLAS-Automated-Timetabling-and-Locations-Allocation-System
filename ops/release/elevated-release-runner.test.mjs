...
test('forbidden operations and cutover fail closed', () => {
	assert.throws(() => runRelease(['--sha', sha, '--release-root', root, '--mode', 'cutover']), (error) => error.code === 'ARGUMENT_REJECTED');
...
test('operator script is manual, elevated, and never registers a task', () => {
	const script = readFileSync(new URL('./Invoke-AtlasElevatedReleasePreflight.ps1', import.meta.url), 'utf8');
	assert.match(script, /IsInRole/);
	assert.match(script, /Program Files.*nodejs.*node\.exe/);
	assert.match(script, /Get-FileHash/);
	assert.match(script, /--mode', 'preflight/);
	assert.doesNotMatch(script, /Register-ScheduledTask|New-ScheduledTask|Start-ScheduledTask/);
	assert.doesNotMatch(script, /ConvertTo-SecureString|password|token|secret/i);
});
