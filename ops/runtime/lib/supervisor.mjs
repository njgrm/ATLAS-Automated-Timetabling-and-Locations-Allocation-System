...
export class Supervisor {
	...
	spawnTargets() {
		...
		for (const target of this.targets) {
			...
			handle.onExit((code, signal) => {
				handle.exited = true;
				return this.onChildExit(target.name, code, signal);
			});
	...
	async onChildExit(name, code, signal) {
	...
	async rollback() {
		...
		this.logger?.info(`Rolling back to release=${previous.releaseLabel} pin=${previous.productPin}`);
		this.persist('degraded', { rollbackTo: previous.releaseLabel, rollbackPin: previous.productPin });
		await this.launchAndAwaitHealthy();
		this.persist('running', { releaseLabel: previous.releaseLabel, productPin: previous.productPin });
		return this.getStatus();
	}
