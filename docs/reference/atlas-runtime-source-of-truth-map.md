...
- EnrollPro owns term structure. `enrollpro-term-contract.service.ts` combines `/integration/v1/school-year` and `/integration/v1/active-term` into a school/year-bound `TRIMESTER` or `QUARTERS` contract with ordered identities, preserved labels, optional dates, active term, and a SHA-256 semantic revision. Unknown formats, duplicate identities, school/year mismatch, and active terms outside the order fail closed.
