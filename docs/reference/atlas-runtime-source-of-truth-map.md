...
- **Runtime supervision contract (RUNTIME-SUPERVISION-C01, 2026-09-12, source integrated; pin-verification correction in flight; live installation pending separate HIGH approval):** `ops/runtime/**` adds a repository-owned supervisor and production host: reviewed pin plus operator-declared exact release identity (ancestor-pin semantics; correction pending), one owner per port (5001/5174) with unknown-listener rejection and no broad kill, production static host/proxy for the built `atlas-client/dist` (SPA fallback; `/api`, `/uploads`, `/enrollpro-api`, `/enrollpro-uploads`, SSE, and WebSocket parity; Vite dev rejected as durable runtime), bounded restart/backoff with duplicate-instance prevention, split liveness vs dependency readiness (`GET /api/v1/health/ready` added alongside the constant `/api/v1/health`), pinned `ROLLOVER_AUTO_SYNC_ENABLED=false`, bounded redacted logs, opt-in supervised crash exit (`ATLAS_SUPERVISED`), reversible start/stop/status/install-preview/uninstall-preview/rollback, and read-only legacy-task inventory. The live 5001/5174 processes remain the unmanaged `EPHEMERAL_DEPLOYMENT` on `d44f29e0` until the separately approved installation.
