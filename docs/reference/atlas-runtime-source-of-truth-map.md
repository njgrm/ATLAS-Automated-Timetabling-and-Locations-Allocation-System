...
| Subject catalog | ATLAS (sole owner) | `Subject` | ATLAS owns schedulable rows and minutes plus persisted ownership contract fields (`outputLabel`, `ownerDepartment`, `qualificationPriority`, `rotationFamily`, `isSystemManaged`) and explicit additional owner-department markers (`requiredFeatures` entries with `OWNER_DEPT:*`); runtime qualification uses multi-department ownership baseline and explicit Teaching Load assignments; SCA-01 (2026-09-07): EnrollPro is NOT a source of subject offerings — the upstream offering refresh action is retired and catalog reads never fall back to another school's scope; year-specific required subjects belong to the SCA-02 Curriculum Requirements authority, not the catalog | Subjects, assignments, templates, timetable |
| Current-year required subjects | ATLAS (sole owner) | `SchoolYearTermConfig`, `SchoolYearOffering`, `OfferingTermAssignment` (user-facing name "Curriculum Requirements") | SCA-02 (2026-09-07): operator-persisted per-(school, year) required subjects with grade/program defaults plus optional section/cohort overrides; term count/identities are operator configuration, never defaulted; all-year rows carry zero term assignments; explicit-empty scopes are distinguishable from missing truth; mutations are version-guarded single-row ops or fingerprinted Serializable batch preview/apply; catalog-compatibility suggestions are unapproved-only. Teaching Load and generation still consume legacy demand paths — the SCA-04 switch is pending, so this authority is configuration-only until then | Subjects (`/subjects/requirements`), teaching-load (future), generation (future), audit |
