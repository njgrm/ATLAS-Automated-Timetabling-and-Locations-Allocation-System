...
Operational backup and faculty identity update (`2026-09-07`, Prompt 06):

- The active database is `atlas_recovery_clean_rebuild_20260905` (Prompt 05B GO;
  Tailnet login restored). `atlas_db` remains preserved and empty.
- Operational backups live outside the repository at
  `D:\ATLAS-database-recovery\backups` (custom-format `pg_dump -Fc` plus
  checksum manifests; PostgreSQL 18 client/server parity enforced). A Windows
  scheduled task (`ATLAS Daily Backup`, daily 22:00, 14-backup retention,
  24-hour RPO) produces and validates them; one disposable restore drill into
  `atlas_restore_drill_20260907_drill01` verified content, runtime smoke, and
  cleanup. Bare `prisma migrate deploy` is forbidden — use `npm run
   migrate:guarded`. WAL/PITR stays behind separate approval and is not enabled.
 - Prompt 06R correction (`2026-09-07`, REVIEW_REQUIRED): faculty login is
   fail-closed (a faculty token requires exactly one same-school active
   non-stale canonical mirror; stale/inactive/ambiguous/cross-school/
   unresolved identifiers are rejected with typed 403 codes and no success
   side effects), the migration gate revalidates the exact archive with a
   current `pg_restore --list` immediately before spawning Prisma, and the
   restore drill guarantees cleanup after target creation. Real candidate
   restoration is a manual operator-controlled procedure (not automated); the
   drill command stays restricted to disposable targets. The `ATLAS Daily
   Backup` task correction (non-interactive S4U identity, battery-agnostic
   start, catch-up on missed runs) is operator-pending: the executor runs
   non-elevated and Windows refused the change (`Set-ScheduledTask`
   `HRESULT 0x80070005`), so nightly RPO still rests on the interactive-only
   configuration until an elevated operator applies the documented correction.
- Faculty login resolves an identifier through the already linked canonical
  mirror when the auth duplicate is null (ambiguous, cross-school, inactive,
  and stale matches rejected). Seeding fills null auth duplicates from the
  mirror without wiping; EnrollPro provisioning hydrates only actually-returned
  fields. The fixed faculty identifier `2000056` is retired: discover a current
  active faculty identifier from linked mirrors at QA time; never recreate it
  and never record passwords in artifacts.
...
- Annual Teaching Load consumer contract v2 (`2026-09-07`, TL-06R3C; ATLAS source only, `PENDING_DEPLOY` until separately authorized): `source.contractVersion=2`; every `assignments[]` row keeps all legacy fields and adds `facultyExternalId` (same-school `FacultyMirror.externalId`), `sectionExternalId` + `sectionName` + `gradeLevelId/Name` + `programType/Code/Name` (same-school+year `SectionMirror`), and `subjectCode/Name`, `subjectOutputLabel`, `minMinutesPerWeek`, `rotationFamily`, `termGroupId`, `termCount` (same-school `Subject`). Enrichment is 3 set-based reads (never N+1); any unresolvable scoped metadata fails closed with `500 EFFECTIVE_CONTRACT_METADATA_INCOMPLETE` and no partial payload; `EMPTY` stays a valid empty v2 contract. Internal IDs are never cross-system keys. Consumer guide: `docs/guides/AIMS_SMART_ANNUAL_TEACHING_LOAD_CONTRACT.md`.
...
- Automated rollover sync: `rollover-automation.service.ts` runs a periodic tick (default 5 minutes) per school. Automation applies `applyRolloverSync` with `initiatedBy: 'system'` only when the codebase's own safety verdict holds — `drift.status === 'atlas-stale'` **and** `drift.recommendedAction === 'RUN_ROLLOVER_SYNC'` **and** zero conflicts **and** zero unacknowledged reconfigured sections (ROLLOVER-GRADED-AUTONOMY-C01, 2026-09-17). `atlas-stale` and `RUN_ROLLOVER_SYNC` are the same condition in `buildDriftState`; the reconfigured term is load-bearing because `applyRolloverSync` hard-rejects unacknowledged reconfigures with `409 SECTION_RECONFIGURATION_REVIEW_REQUIRED`. Any other preview is notification-only: the tick publishes one `ROLLOVER_DETECTED` event (`domain: 'integration'`, `audience: 'PRIVILEGED'`) per observed drift-status transition, carrying `driftStatus`, `recommendedAction`, `driftMessage`, and `conflictCount`, deduped on the drift status rather than on `notifyOnce`'s `type:lastResult` axis, and never for `aligned`. Transient failures use bounded exponential backoff. **The automation never auto-archives**: it must not reach `archiveSchoolYear`, `archiveSupersededYearsForRecovery`, `archiveAndSyncActiveYear`, or `applyTestYearRecovery`, so the post-sync archive loop, the archive-resolvable self-heal, the pending-archive marker retry, and the test-mode collision auto-clear are retired from the tick; superseding the previous year, completing a pending recovery, resetting a dummy year, and clearing marked test data remain explicit operator actions on the existing `/admin/year-setup` and `/runtime/rollover-{recovery,archive,sync}` surfaces, whose service-level code and durable markers are unchanged.
- Env vars: `ROLLOVER_AUTO_SYNC_ENABLED`, `ROLLOVER_AUTO_SYNC_INTERVAL_MS` (default `300000`), `ROLLOVER_AUTO_SYNC_MAX_BACKOFF_MS` (default `1800000`). The service default is enabled (`!== 'false'`), but the supervised runtime is the operative control: since ROLLOVER-GRADED-AUTONOMY-C01 (2026-09-17) the `ops/runtime` contract no longer hard-pins the value, so `invariants.ROLLOVER_AUTO_SYNC_ENABLED` in `ops/runtime/runtime-contract.json` is a real, reviewable control that is validated as exactly the string `'true'` or `'false'` (any other value fails `RUNTIME_CONTRACT_INVALID`) and passed through unchanged to both children, while `invariants.ATLAS_SUPERVISED` stays hard-pinned to `'true'`. The shipped contract still declares `false`, so the live runtime keeps the automation off; switching it to `true` authorizes unattended production data mutation (faculty and section sync, Teaching Load cycle upsert, mirror upsert, audit writes) and is therefore a HIGH action needing its own reviewed approval — and it must not precede `ROLLOVER-DETECTED-CLIENT-C01`, because the client `NOTIFICATION_EVENT_TYPES` allowlist still drops `ROLLOVER_DETECTED`.
...
- **Teaching Load carry-forward preview (TL-RR01, 2026-09-11, source integrated; served by the supervised live release since 2026-09-12 (now `54dce67b`), carry-forward apply not authorized):** `/admin/year-setup` adds an optional, zero-write "Start from last year" preview (`CarryForwardReviewPanel`) backed by `POST /api/v1/teaching-load/carry-forward/preview` (operator JWT, privileged role, strict body, actor-school scoped). It resolves exactly one active non-archived target year and one same-school archived source year, matches faculty by stable external identity and sections by canonical grade+program+normalized name, intersects archived assignments with the current `DERIVED_DEMAND_V2` pair identities, re-resolves current qualification/minutes/hard caps, and reports typed per-row reasons (never calling a skipped row "carried"). A separate privileged `POST /api/v1/teaching-load/carry-forward/apply` is implemented but not invoked by this stream: it revalidates source/target revisions and the fingerprint inside one Serializable transaction, writes only empty target `SubjectSectionOwnership`/`FacultySubject` projections, refreshes the target `TeachingLoadCycle` once, and writes exactly one `TEACHING_LOAD_CARRY_FORWARD` audit, aborting atomically with zero writes on drift. The client exposes no apply action. Archived source rows are never mutated. **TL-RR01R correction (`2026-09-11`):** grades resolve from authoritative `SectionMirror.gradeLevelId` via the established EnrollPro normalization (never `displayOrder`); an unconfigured effective workload policy fails preview and apply closed with a typed `409 WORKLOAD_POLICY_UNCONFIGURED` (revalidated inside the Serializable apply transaction; no infinite hard cap); and apply requires a strict positive authenticated `userId` (typed `403 ACTOR_USER_REQUIRED` with zero reads/writes).
...
- **Persisted term-authority catch-up (RR-TERM-CACHE-C01/C01R, 2026-09-11 → 2026-09-12, source integrated; served by the supervised live release since 2026-09-12 (now `54dce67b`), term-cache apply pending):** year drift and persisted ordered-term authority are separate, truthful states. `GET /api/v1/runtime/rollover-status` adds `termAuthority` (`PERSISTED_CURRENT | MISSING | PERSISTED_STALE | UPSTREAM_UNAVAILABLE | INVALID_UPSTREAM_CONTRACT | CACHE_INVALID | YEAR_NOT_MIRRORED | PERSISTED_UNVERIFIED` with `needsRepair`/`repairAction`) resolved independently from `drift.recommendedAction`, which never implies term readiness; `teachingLoadResetRequired` is namespaced under `teachingLoadReset { required, scope: 'DUMMY_YEAR_RESET_PREVIEW', applicable, reason }` and is only true when the typed dummy/test-data reset path actually applies. Zero-write `POST /api/v1/runtime/term-authority/preview` validates the live EnrollPro ordered contract, binds the active mirror's cache state, and returns terms, semantic revisions, a SHA-256 fingerprint, and exact confirmation `SAVE_TERM_AUTHORITY_<school>_<year>`. Privileged, actor-school-scoped `POST /api/v1/runtime/term-authority/apply` requires the exact confirmation and fingerprint, re-fetches/revalidates the live contract and mirror/cache inside one Serializable transaction, and writes ONLY the active mirror's `termContractCache`/`termContractCachedAt` plus one `TERM_CACHE_SYNC_APPLIED` audit; drift/scope mismatch is a typed 4xx with zero writes, identical replay is idempotent with no second audit, and a reachable `409 ACTIVE_TERM_UNRESOLVED` never invalidates the ordered structure. The `RolloverGuidanceCard` exposes exactly one `Save terms` repair action (preview dialog → exact-confirmation apply) and never calls the broad faculty/section rollover apply. The RR-TERM-CACHE-C01R correction (2026-09-12) makes both routes JWT-only privileged operator actions (the system token is rejected with 401; strict actor user/school authority returns typed 403 codes before any service/upstream/DB dispatch), removes every term-authority `schoolId = 1` default (`previewTermCacheSync`/`applyTermCacheSync` require an explicit positive school; `RolloverGuidanceCard` requires an explicit actor school and omitted-prop callers use the fail-closed `ActorScopedRolloverGuidanceCard`, which resolves `/auth/me` and dispatches nothing until the actor school is a strict positive integer; actor-school changes clear preview/confirmation/errors and late previews are dropped), and re-elects the complete same-school active, non-archived mirror set through the transaction client inside the apply's Serializable transaction (`ACTIVE_YEAR_UNAVAILABLE` on zero rows, `ACTIVE_YEAR_AMBIGUOUS` on more than one; a previously selected row staying active is not sufficient authority). The RR-TERM-CACHE-C01R2 correction (2026-09-12) binds the client actor-school resolver cache (`resolveActorSchoolId`) to the exact authenticated token epoch: logout, session expiry, same-tab re-login, bridge-token replacement, and actor-school switching invalidate or revalidate the cached school before any scoped request; a late `/auth/me` response from an obsolete session can neither return, seed, nor overwrite the current scope; and no token means `null` with zero dispatch (no school-1 fallback). The deployment packet `docs/prompts/rr-term-cache-live-deploy-preview-2026-09-12.md` (stop-then-start swap) is **SUPERSEDED** by the confirmed outage (Tailnet 502, no listeners on 5001/5174); the replacement `docs/prompts/actor-scope-deploy-restore-2026-09-12.md` restores the service on empty listeners (no incumbent, no swap claim), pins the integrated product `d44f29e0`, and uses `fdd0c8c7` only as a startable fallback. The live catch-up remains a separately approved HIGH action; `PENDING_DEPLOY`.
- **Actor-school/year scope closure (ACTOR-SCOPE-C01, 2026-09-12, deployed live through the `d44f29e0` restore and now served by the supervised live release `54dce67b`; Stage C acceptance closed 2026-09-12):** the three runtime READ routes are actor-gated: `GET /api/v1/runtime/context`, `GET /api/v1/runtime/rollover-status`, and `GET /api/v1/runtime/rollover-recovery/classify` require an explicit positive `schoolId` (typed `400 INVALID_PARAM` otherwise; never default 1), JWT callers may read only their authenticated school (typed `403 CROSS_SCHOOL_DENIED`; missing actor school `403 SCHOOL_SCOPE_REQUIRED`), system/integration tokens retain documented access only with an explicit school, and the two operator status/recovery routes additionally require a privileged role while same-school faculty keep `/context`. Every client actor/tenant-sensitive helper (`resolveActiveSchoolYearContext`, `promoteActiveSchoolYearContext`, `fetchAtlasRuntimeContext`, `fetchRolloverStatus`, and the rollover/recovery/archive helpers) now requires an explicit positive `schoolId` from the authenticated actor (`resolveActorSchoolId`), bound to the token epoch via `lib/actor-scope-session.ts` + the `lib/auth.ts` epoch subscription: mounted consumers drop the previous school to `null` immediately on any token mutation, clear school/year-dependent state, dispatch zero scoped requests until the new actor school resolves, and discard late session-A responses in both orderings. Non-listed runtime mutation routes still use the defaulting `parseSchoolId`, and pilot-school constants remain in non-listed pages (`MapEditor`, `MapView`, `SpecializationMapping`, `PublicPublishedSchedule` public-by-contract, `coverage.ts`, `CreatePlaceholderDialog`) — recorded as a separate successor authorization lane, not part of the accepted contract. Deployed and accepted 2026-09-12.
- **Companion SSO (COMPANION-SSO-C01, 2026-09-13, source integrated at `c989f03d`; live activation and environment configuration remain separately gated):** the ATLAS side of the hub model. Flow A: `GET /api/v1/auth/enrollpro/callback` validates the 43-char code, exchanges it server-to-server at EnrollPro (`${ENROLLPRO_BASE_URL}/auth/companion-sso/atlas/exchange`, Bearer `ENROLLPRO_SSO_CLIENT_SECRET`; legacy alias `ATLAS_SSO_CLIENT_SECRET`), requires `companion=atlas`, an upstream role intersecting `{SYSTEM_ADMIN, HEAD_REGISTRAR, CLASS_ADVISER, TEACHER}`, an existing active `atlas_auth_accounts` row (exact `employeeId` then `accountName`; never auto-creates), the faculty canonical-mirror eligibility gate, and school/year parity with the active `EnrollProSchoolYearMirror`; it then mints the same local JWT and redirects to `/auth/sso/callback` with the token in the URL fragment (stripped via `history.replaceState` before navigation). Flow B: `POST /api/v1/auth/sso/authorize` (JWT + privileged role, strict `client_id`/`redirect_uri`/`response_type`/`state` validation against `ENROLLPRO_SSO_CALLBACK_URL`) issues 32-random-byte/43-char codes stored **hash-only** (`companion_sso_codes.codeHash`, SHA-256, 60s TTL, audience `enrollpro`, exact redirect binding) and `POST /api/v1/auth/sso/exchange` consumes them atomically with a constant-time reverse-secret compare (`ATLAS_SSO_REVERSE_CLIENT_SECRET`; legacy alias `ENROLLPRO_REVERSE_CLIENT_SECRET`) and returns the ATLAS identity assertion whose identity.roles are mapped from the persisted local role to the exact EnrollPro RoleEnum (officer/admin/SYSTEM_ADMIN -> SYSTEM_ADMIN, faculty -> TEACHER) and whose non-empty firstName/lastName are resolved from persisted identity (faculty mirror, else accountName); an unrepresentable role or unassertable name fails typed 403 (COMPANION_SSO_ROLE_UNMAPPABLE / COMPANION_SSO_IDENTITY_NAME_UNAVAILABLE) with zero assertion and zero success-audit rows (COMPANION-SSO-C03, 2026-09-15). Client: `/auth/sso/callback` + `/auth/enrollpro/authorize` pages, `Login.tsx` `returnUrl` resume, and the Integrated Systems AppShell area (AIMS/SMART/ATLAS/MRF; ATLAS non-clickable current; EnrollPro via `VITE_ENROLLPRO_SSO_START_URL` or derived reverse/start; AIMS/SMART/MRF disabled; no raw companion dashboard links). Existing local login, `/auth/me`, and bridge-token behavior are unchanged. Live activation requires EnrollPro-side registration/secrets and is a separate HIGH action.
- **Current live runtime (installed 2026-09-15 by the `ENROLLPRO-PROXY-RECOVERY-LIVE` recovery; re-verified read-only 2026-09-16):** the live 5001/5174 processes are the supervised release `54dce67b8392cbce09aa810813c37f9c87a67159` (label `atlas-d44f29e0`, reviewed ancestor pin `d44f29e04d359ad9b18e4443b0fd4fed1daeaecd`), from source directory `D:\ATLAS-runtime-supervised-54dce67b-20260914`. Verified read-only at capture: one listener per port (5001 server, 5174 production host), local `/api/v1/health` and `/api/v1/health/ready` (database `ok`), host `/__host/live` and `/__host/ready` (upstream `ready`), and Tailnet health all `200`; supervisor `state: running`, `restartFailures: 0`, invariants `ATLAS_SUPERVISED=true` and `ROLLOVER_AUTO_SYNC_ENABLED=false`; durable runtime env `D:\ATLAS-runtime-config\atlas-server.env`. Rollback remains the supervised reset to `9d293879` (`D:\ATLAS-runtime-supervised-20260912`), with the non-supervised `d44f29e0` fallback at `D:\ATLAS-runtime-fallback-d44-20260912` retained as a manual last resort. Open item: the boot-recovery task is not enumerable from a non-elevated session, so durable restartability still requires one elevated read-only confirmation before any HIGH runtime action.
- **Durable runtime environment keys (operator decisions 2026-09-17):** `D:\ATLAS-runtime-config\atlas-server.env` carries two keys that are outside the reviewed product contract. (1) `ATLAS_AUTH_DISABLE_RATE_LIMIT` is a real code toggle (`local-auth.service.ts:12-14`) that bypasses ALL login rate limiting; the live value is `true` (its recorded fingerprint hashes to `sha256("true")`, `b5bea41b…`). Operator decision: KEEP it for the consolidated-deployment acceptance login, then DISABLE it before publication; it is an operational affordance, not a production invariant, and removing it is a separately reviewed HIGH env change. (2) `ATLAS_DEFAULT_SCHOOL_ID` is VESTIGIAL: the login-provisioning fallback that consumed it was intentionally removed (`CHANGELOG.md:2129`, `:2141`, `:4550` — "Zero references to `ATLAS_DEFAULT_SCHOOL_ID` in auth service"), no code reads it, and the live value is `1` (hashing to `sha256("1")`, `6b86b273…`). Operator decision: DELETE it from the durable env and from `atlas-server/.env.example` (the example line is removed in the same 2026-09-17 decisions commit); it must not survive as an apparent live fail-open school default.
- **Runtime supervision contract (RUNTIME-SUPERVISION-C01, 2026-09-12, source integrated at `0ec3b8f7`; live-installed via the deploy-as-restore `RUNTIME-SUPERVISION-LIVE-INSTALL-RESTORE` on 2026-09-12):** `ops/runtime/**` adds a repository-owned supervisor and production host: reviewed ancestor pin plus operator-declared exact release SHA (`ATLAS_RUNTIME_RELEASE_SHA`), one owner per port (5001/5174) with unknown-listener rejection and no broad kill, production static host/proxy for the built `atlas-client/dist` (SPA fallback; `/api`, `/uploads`, `/enrollpro-api`, `/enrollpro-uploads`, SSE, and WebSocket parity; Vite dev rejected as durable runtime), bounded restart/backoff with duplicate-instance prevention, split liveness vs dependency readiness (`GET /api/v1/health/ready` added alongside the constant `/api/v1/health`), contract-declared `ROLLOVER_AUTO_SYNC_ENABLED=false` (a real reviewable control since ROLLOVER-GRADED-AUTONOMY-C01; the validator requires the string `"true"` or `"false"` and keeps `ATLAS_SUPERVISED` hard-pinned to `"true"`), bounded redacted logs, opt-in supervised crash exit (`ATLAS_SUPERVISED`), reversible start/stop/status/install-preview/uninstall-preview/rollback, and read-only legacy-task inventory. The live 5001/5174 processes were the supervised release `3d916b26` (reviewed ancestor pin `d44f29e0`) started by `ops/runtime/cli.mjs` from `D:\ATLAS-runtime-supervised-3d916b26-20260912` (deployed 2026-09-12 under the `TT-TL-RUNTIME-ACCEPTANCE` approval), with the boot task `ATLAS-Runtime-Supervisor` (ONSTART, `PT0S`) re-pointed to that release and legacy `ATLAS-DevServer-Temp2` disabled; the prior unmanaged `EPHEMERAL_DEPLOYMENT` is superseded. That release is historical: since 2026-09-15 the live 5001/5174 processes are the supervised `54dce67b` release (see the current live runtime bullet), and `9d293879` remains the supervised rollback.
- **EnrollPro proxy origin recovery (ENROLLPRO-PROXY-RECOVERY-C01, 2026-09-14, source-only):** the supervised EnrollPro upstream is one explicit, durable, normalized HTTPS origin. `ops/runtime/lib/supervisor.mjs::buildTargets` now resolves `ENROLLPRO_PROXY_ORIGIN` from the fully composed child environment (`{ ...inheritedEnv, ...durableEnvValues, ...invariantEnv }`), so a durable value in `ATLAS_RUNTIME_ENV_FILE` overrides a conflicting inherited process value (the previous code read the raw inherited env and silently used the `http://127.0.0.1:5000` contract default). A single shared normalizer (`ops/runtime/lib/enrollpro-origin.mjs`, reused by the supervisor and the production host) accepts only absolute normalized `http(s)` origins and rejects credentials, paths, query strings, fragments, non-`http(s)` schemes, and unparseable values with typed `ENROLLPRO_PROXY_ORIGIN_INVALID`; blank values are missing. The exact CLI launch paths (`start`, and `rollback` when it relaunches) gate on an explicit origin and fail closed before constructing or spawning any child with typed `ENROLLPRO_PROXY_ORIGIN_MISSING` (exit 1, JSON `{code,...}`, zero spawn) when neither the durable file nor the inherited environment supplies it; `stop`/`status` remain usable, and the reviewed dev default applies only outside the launch gate. `ops/runtime/host.mjs` requires `ATLAS_HOST_ENROLLPRO_TARGET` (no `|| ATLAS_HOST_API_TARGET` fallback) and validates it with the same normalizer. Proxy semantics are unchanged (`/enrollpro-api/x?y=1 -> <origin>/api/x?y=1`, `/enrollpro-uploads/x -> <origin>/uploads/x`, method/body/status/body/headers, SSE, WebSocket upgrade); an unreachable EnrollPro upstream returns a bounded `502 {"code":"UPSTREAM_UNREACHABLE"}` for that request while ATLAS `/__host/live`, `/__host/ready`, `/api`, and the SPA fallback stay 200 — EnrollPro availability is not an ATLAS readiness prerequisite. Client companion navigation is fail-closed from explicit configuration only: `VITE_ENROLLPRO_URL` (trimmed, trailing slashes removed) yields the reciprocal `${base}/dashboard` back link and the `${base}/personnel/login` bridge-logout redirect; `VITE_ENROLLPRO_SSO_START_URL` wins for the reverse-SSO start, else `${base}/api/auth/companion-sso/atlas/reverse/start`, else `null` and EnrollPro renders as disabled plain text with no href; the retired raw-IP fallbacks are removed and unresolved configuration never navigates externally. **Live-recovery boundary (HIGH, performed 2026-09-15 under the `ENROLLPRO-PROXY-RECOVERY-LIVE` approval, `LIVE_ACCEPTED` 20/20):** the durable `ENROLLPRO_PROXY_ORIGIN` and `ENROLLPRO_API` were set in `D:\ATLAS-runtime-config\atlas-server.env`, release `54dce67b` was built and installed, and the supervisor-owned 5001/5174 processes were relaunched on that release. See the current live runtime bullet for the verified identity. A future change to the durable origin remains a separately approved fingerprinted HIGH action with its own preview, approval, and rollback. `atlas-server/src/app.ts` CORS defaults and `atlas-client/qa-artifacts/**` are explicitly out of scope for this stream.
...
- DASH-RESILIENCE-C01 (`2026-09-11`): the readiness summary is the SINGLE Dashboard load pipeline — the legacy client-side fan-out fallback is removed from normal error recovery. The service returns each failed domain as an explicit `available: false` block with `null` values; it never substitutes `0`, `[]`, or `NONE` for an unavailable domain, and `resolveDashboardLifecycle` holds at `SETUP` when a domain value is unknown. A genuine persisted zero stays `0` with `available: true`. The summary also exposes the typed `activeTerm`, so a reachable HTTP 409 `ACTIVE_TERM_UNRESOLVED` stays reachable with its code/message and never collapses to `enrollpro-unreachable`. The client retains the last same-school snapshot on a transient refresh failure (concise degraded state with one retry) and routes HTTP 401/403 to the canonical session-expired / blocked-scope UX without dispatching any legacy or fallback request.
- UX-C01 derived setup UX (`2026-09-11`): the superseded annual Curriculum Requirements / Decision Workspace surfaces are removed from the routine operator journey. `/subjects/requirements` and `/subjects/decision-workspace` are non-mutating replace-redirects to `/subjects?context=derived-setup`; the page components and their mutation UI are deleted, and no normal route, link, or client source dispatches the retired `/curriculum-requirements` API. `dashboard-readiness.service.ts` no longer imports `evaluateCurriculumReadiness`/`school-year-offering`; it resolves setup readiness from `buildDerivedDemand` and exposes a typed `derivedDemand` domain (`available`, `ready`, `yearLabel`, `revision`, ordered `termStructure`, `blockers`, `subjectMetadataExceptions`, `totals`) instead of `curriculum`. A typed blocker is `available: true, ready: false` and holds `resolveDashboardLifecycle` at `SETUP`; only a datasource failure is `available: false`. The Dashboard and Timetable now consume the same derived blockers as generation and Teaching Load, and the Dashboard presents them in operator order (EnrollPro year/terms, Subject metadata exceptions, derived demand totals, Teaching Load coverage, generation state) with one smallest true repair. A new read-only `GET /api/v1/derived-demand/:schoolId/:schoolYearId/readiness` (actor-school scoped, privileged roles) backs the Dashboard derived-setup readiness aggregation and returns the same typed shape; the Timetable readiness hook consumes the canonical generation diagnostic (`/generation/:schoolId/:schoolYearId/readiness/diagnostic`) instead, so only the complete canonical decision can enable generation. `Subjects.tsx` is the setup surface for ATLAS-owned metadata (participation, grade/program scope, weekly minutes, rotation, room needs) and labels the active EnrollPro year and ordered terms as read-only source data.
...
- The dashboard exposes `Check for updates`, which reruns the single readiness-summary pipeline without introducing a new backend endpoint.
...
| `/` | Dashboard | Setup snapshot, campus map view, room overlay health | `/dashboard/readiness-summary?schoolId=:schoolId` for the first readiness snapshot; campus preview still uses detail endpoints such as `/subjects`, `/faculty`, `/sections/summary/:schoolYearId`, `/generation/:schoolId/:schoolYearId/runs/latest/timetable`, and `/room-schedules/.../rooms/:id` for room overlay drilldown | Mixed ATLAS service aggregation over runtime context, map inventory, subject/faculty/section mirrors, and latest generation run; EnrollPro is used only for runtime verification/branding refresh, not as the first dashboard render dependency | Dashboard can look healthy while generation still fails; it is not a phase-closure signal. The route surfaces a compact source badge (`Verified live`, `Checking source`, `Using saved data`, `No saved data`, or `Partial data`). Since DASH-RESILIENCE-C01 (`2026-09-11`) it uses `/dashboard/readiness-summary` as its only load pipeline: a failed domain renders as unavailable (never `0`), a transient same-school refresh failure retains the last snapshot with a concise degraded state and one retry, and HTTP 401/403 route to the canonical session-expired / blocked-scope UX without dispatching any legacy or fallback-school request. Since UX-C01 (`2026-09-11`) setup readiness is the canonical `derivedDemand` domain (active EnrollPro year + verified ordered terms + ATLAS Subject scheduling metadata) rather than Curriculum Requirements readiness; a blocked derivation holds the lifecycle at `SETUP` and the next-step card offers only the smallest true repair (Year Setup, Subjects, Teaching Load, or generate). |
| `/subjects` | Subjects | Subject catalog management, ownership inspection, and subject coverage drilldown | `/subjects`, `/subjects/:id`, `/subjects/:id/archive`, `/subjects/:id/reactivate`, `/subjects/:id/delete-preview`, `/subjects/:id/delete-apply`, `/faculty-assignments/summary` | ATLAS `Subject` is authoritative; department ownership is the active qualification baseline and manual Teaching Load placement remains authoritative | Page load is read-only (no passive seed mutation). SCA-01 (2026-09-07): the catalog read resolves the actor school through the authenticated runtime context with no school-1 fallback (bounded loading/unavailable state while unresolved); reads order by name with no isSeedable priority; isSeedable is hidden bootstrap metadata, not timetable demand; the upstream offering refresh action is retired (its apply is permanently blocked and EnrollPro does not supply subject offerings). Since UX-C01 (`2026-09-11`) this page is the lone setup surface for ATLAS-owned subject scheduling metadata (participation, grade/program scope, weekly minutes, rotation, room needs); the active EnrollPro year and ordered terms are shown read-only, and the legacy annual requirements page is a replace-redirect only. Create/patch/archive/reactivate/delete paths stay version-guarded and actor-school-scoped. Delete blockers now distinguish active assignments vs historical rows and expose archive/cleanup paths. Global teaching-load reset controls were moved out of this page to `/teaching-load` (legacy `/assignments` redirect retained); this page keeps subject-scoped remediation only. |
...
| Subject catalog | ATLAS (sole owner) | `Subject` | ATLAS owns schedulable rows and minutes plus persisted ownership contract fields (`outputLabel`, `ownerDepartment`, `qualificationPriority`, `rotationFamily`, `isSystemManaged`, scheduling disposition/scope, room needs) and explicit additional owner-department markers (`requiredFeatures` entries with `OWNER_DEPT:*`); runtime qualification uses multi-department ownership baseline and explicit Teaching Load assignments. EnrollPro is not a source of subject offerings; its authority is the active school year and ordered term identities. Current-year demand is derived by DEMAND-C01 from those terms plus ATLAS subject metadata and active section mirrors; retired Curriculum Requirements rows are not runtime authority. | Subjects, assignments, Teaching Load, templates, timetable, generation |
| Current-year required subjects | Derived contract: EnrollPro owns active year and ordered terms; ATLAS owns subject scheduling metadata and active section mirrors | `EnrollProSchoolYearMirror.termContractCache`, `Subject`, `SectionMirror`; resolved by `buildDerivedDemand` | DEMAND-C01 deterministically derives per-term timetable lines and annual Teaching Load pairs. Missing or stale term authority, subject scope, rotation metadata, or section scope fails closed; ATLAS does not ask operators to duplicate the curriculum in a separate Curriculum Requirements page. `SchoolYearTermConfig`, `SchoolYearOffering`, and `OfferingTermAssignment` are legacy/readable data only and are not selected as current-year demand truth. | Subjects, Teaching Load suggestions/reconciliation, generation, timetable, readiness, publication freshness |
...
### 8k. Published Revision Immutability Contract (2026-09-16, C08/C08R1)

- A published revision is a historically reproducible artifact. Publication freezes subject, faculty, section, room, building, specialization, cohort, display-slot, special-event, policy, and ordered-term identity into the base `PublishedScheduleRevision.metadata.publishedIdentitySnapshot` JSON. No schema migration was introduced.
- The snapshot is built from a preflight read and bound to the same `Serializable` transaction (advisory lock + run-version CAS) that persists the publication; a covered-input change between preflight and write fails closed with zero writes.
- Published reads and every official export (public schedule, authenticated faculty schedule, Section/Class Program, Teacher Program, Room Program, Summary workbook, class-program matrix) resolve frozen values first and never rehydrate human-readable identity from current authority tables.
- Legacy publications without a snapshot return the honest typed state `snapshotState: 'LEGACY_LIVE_PROJECTION'` plus a `snapshotGaps` list; they are never reported as immutably reproduced.
- Archived/published per-term reads and official exports resolve the requested term through the frozen ordered-term contract, so they do not depend on the live `EnrollProSchoolYearMirror` active/non-archived term-cache. The live path remains for unpublished/draft work.
- `GET /api/v1/schools/:schoolId/schedules/published/:termId` (and its `/sections`, `/faculty`, `/rooms` siblings) treat `termId` as term identity within the resolved published school year — never as a `schoolYearId` alias. Invalid/absent/out-of-contract terms fail closed with the typed `academic-term` codes.
- Current-year reads report `isActiveSchoolYear: true` / `isHistorical: false`; other years report the inverse; an archived year is never elected current.
- Routine faculty/subject/room/policy synchronization must never set a published run to `FAILED` or `isPublished: false`. Drift is recorded as a typed, audited `GENERATION_RUN_PUBLICATION_DRIFT_DETECTED` marker that copies through the run's `isPublished` state, so a superseded run (`isPublished: false` with informational `publishedAt`/`publishedBy`) is not resurrected into a second current published run.
...
TERM-SUBJ-C01 EnrollPro term and Subject scheduling authority (`2026-09-10`, `REVIEW_REQUIRED`):
- The Subjects page now requests the authenticated `GET /api/v1/subjects/scheduling-authority?schoolYearId=` production route after actor-school and active-year resolution. The route returns the catalog even when term authority blocks, but never invents rotation labels without a verified contract.
- EnrollPro owns term structure. `enrollpro-term-contract.service.ts` combines `/integration/v1/school-year` and `/integration/v1/active-term` into a school/year-bound `TRIMESTER` or `QUARTERS` contract with exact supplied ordered identities and labels, optional dates, active term, and a SHA-256 semantic revision over those exact values. Flat responses must explicitly supply every identity and label; ATLAS never invents or rewrites either. Canonical keys are comparison-only: duplicate canonical identities fail closed, while a case-insensitive active-term match resolves back to the exact ordered-contract identity before caching or emission. Missing entries, unknown formats, duplicate identities, school/year mismatch, and active terms outside the order fail closed.
- A verified live contract is persisted only in the exact `EnrollProSchoolYearMirror.termContractCache` plus `termContractCachedAt`. An exact school/year/hash match may be used as visibly degraded saved data; missing, cross-school, cross-year, or tampered cache state blocks.
- `Subject.schedulingDisposition` is the Subject catalog's scheduling-metadata field with activation state `PENDING_DERIVED_DEMAND_INTEGRATION`. It is readable state only: generation, timetable demand, and Teaching Load still enforce their own HG-specific rules, so the scheduling view makes no demand/Teaching Load claim and exposes no `demandProjection`, `createsTimetableDemand`, or `createsTeachingLoad`. Existing rows default to scheduled teaching; migration backfill and controlled bootstrap change only the exact code `HG` to reference-only (a non-HG subject named "Homeroom Guidance" stays scheduled).
- Explicit Subject rotation family/order resolves against the verified term order. Missing family/order, duplicate family position, and out-of-range position are actionable Subject issues, not inferred defaults.
- Subject POST/PATCH reject deferred `schedulingDisposition` (`PROTECTED_SCHEDULING_DISPOSITION`), EnrollPro-owned term count/format/identities/labels/dates, and protected bootstrap metadata; ordinary creates persist `SCHEDULED_TEACHING` and client create/edit payloads omit the field. Generation, allocation, timetable demand, Teaching Load, Dashboard, Year Setup, publication, and legacy Curriculum Requirements consumers remain unchanged in this cycle.

TERM-CONSUME-C02 ordered structure vs active-term resolution (`2026-09-11`, `REVIEW_REQUIRED`):
- `enrollpro-term-contract.service.ts` now models the authoritative ordered term structure and the current active-term resolution as SEPARATE authority results. The ordered structure (school, year, `TRIMESTER`/`QUARTERS`, exact ordered identities/labels/dates, SHA-256 semantic revision over the structure only) is required. The active term is a typed state: `RESOLVED`, `UNRESOLVED`, `UNAVAILABLE`, or `CONTRACT_INVALID`.
- A valid `/integration/v1/school-year` 200 plus a reachable `/integration/v1/active-term` 409 `ACTIVE_TERM_UNRESOLVED` returns a `VERIFIED_LIVE` structure with `activeTerm: null` and `activeTermState = UNRESOLVED`; it never collapses to `ENROLLPRO_UNREACHABLE`. An active-term network/HTTP failure preserves the structure and reports availability separately. An active-term 200 naming an identity outside the order, or a mismatched school/year, fails typed.
- The semantic revision intentionally excludes active-term state, so a resolved structure and an unresolved structure over the same ordered terms share a revision and cache identity.
- Passive reads are zero-write. `resolveEnrollProTermContract` / `resolveTermContractWithDependencies` never create, repair, or update the cache. `GET /api/v1/subjects/scheduling-authority` may use a verified live structure or an exact-school/year validated cache, but performs zero Prisma writes (proven with an instrumented mounted route plus a rolled-back positive control). A cache is used as visibly degraded `VERIFIED_CACHED` only for the exact school/year + matching semantic revision; missing, cross-school, cross-year, or forged state blocks without synthesizing `T1`, labels, counts, or dates.
- `syncActiveTermContractAuthority` is the ONLY cache writer. It is invoked solely through the explicit actor-scoped rollover path: `POST /runtime/rollover-sync/apply` passes `syncTermContract: true` to `applyRolloverSync`, which calls it after the exact active mirror is established. It binds the cache to school, year, semantic revision, and verification time, and is idempotent (a replay with the same revision performs zero writes). Live year 9 / `2030-2031` is the current 200/409 reference.
- `GET /subjects/scheduling-authority` and the `TermAuthority` client contract keep the exact ordered identities; the client renders the active badge with a null-safe `activeTerm` access. The superseded operator-owned Curriculum Requirements workflow is never populated from this authority; DEMAND-C01 consumes it in its own successor stream.

DASH-RESILIENCE-C01 typed Dashboard term state (`2026-09-11`, `REVIEW_REQUIRED`):
- The Dashboard path shares the same typed vocabulary. `fetchEnrollProActiveTerm` preserves a reachable non-2xx contract result: HTTP 409 `ACTIVE_TERM_UNRESOLVED` returns `source: 'enrollpro-unresolved'`, `reachable: true`, `verified: false`, `code: 'ACTIVE_TERM_UNRESOLVED'`; other non-2xx statuses stay `enrollpro-unreachable` while preserving any upstream code. `fetchEnrollProActiveTerm` is a generic adapter, so it does not synthesize an ordered structure; a typed 409 with an unknown code is `enrollpro-contract-drift`.
- School-year verification and active-term resolution remain independent: a verified `/integration/v1/school-year` 200 with an unresolved active term keeps the actor school's saved ATLAS counts available and exposes the typed term state on `/dashboard/readiness-summary` via the added `activeTerm` field. `published-schedule` and `workbook-export` still treat an unverified/unresolved active term as `TERM_FILTER_NOT_READY`. BENEFICIARY-EXPORT-PARITY-C05 narrows the semantic: an absent `termIndex` on an official export is now a separate typed transport rejection (`TERM_INDEX_REQUIRED`, 400, zero bytes), while `TERM_FILTER_NOT_READY` means only that the selected source run's entries lack per-entry `termIndex` identity, or that an `active` request cannot be verified from the persisted ordered-term authority.

DEMAND-C01 canonical derived demand authority (`2026-09-11`, `INTEGRATED`):
- Demand authority is one deterministic read-only contract in `atlas-server/src/services/derived-demand.service.ts`, derived from the sole active non-archived EnrollPro school year and persisted verified ordered terms, active ATLAS section mirrors, and ATLAS Subject scheduling metadata. `SchoolYearOffering`, `OfferingTermAssignment`, and `SchoolYearTermConfig` are not selected as current-year demand truth.
- A timetable identity is `school/year/term/subject/section`; a Teaching Load identity is the unique `school/year/subject/section` pair across terms. Scheduled Subjects occur in every ordered term, validated rotating members occur only in their configured term, and reference-only Subjects create neither timetable lines nor Teaching Load pairs.
- Rotation authority is complete per active normalized grade/program scope. A family present in a scope must cover every configured position; missing, duplicate, and out-of-range positions fail typed, while disjoint scopes may reuse an order.
- The `DERIVED_DEMAND_V2` semantic revision binds exact ordered terms, active sections, Subject scheduling/room semantics, and `periodLengthMinutes`. Reads are canonical and stable under source ordering; supplied transaction clients govern all persisted authority reads.
- Teaching Load reconciliation, timetable summary/preview, and generation assembly consume and expose this same revision. Projection drift and unavailable authority fail closed rather than becoming empty demand.

TL-SUGGESTION-C03R2 canonical Teaching Load suggestion authority (`2026-09-14`, `INTEGRATED`):
- `buildDerivedDemand` is the sole current-year `(subjectId, sectionExternalId)` pair authority for Teaching Load suggestions, staffing-needs reporting, and over-cap redistribution. `REFERENCE_ONLY`, HG, ARAL, grade/program-mismatched, and otherwise outside-demand ownership never becomes ordinary suggested demand or credited teaching minutes.
- Existing ownership outside canonical demand remains visible as a bounded diagnostic and is excluded from proposed moves. Qualified active zero-load faculty, including Filipino and ESP candidates, remain eligible through persisted qualification, department, cross-department, program, and workload-policy authority.
- Reviewed proposal apply and over-cap apply re-resolve the canonical derived-demand revision through the transaction client before any protected write. Over-cap apply explicitly requests Serializable isolation; a changed revision or pair authority fails with the typed stale contract and zero ownership, `FacultySubject`, cycle, audit, or notification writes.
- These source guarantees authorize no live Teaching Load mutation. Deployment and each suggestion, redistribution, or carry-forward apply remain separately previewed, independently reviewed, and explicitly approved HIGH actions.

DEMAND-C01R2 ordered-term consumer closure (`2026-09-11`, `INTEGRATED`):
- `academic-term.service.ts` is the server authority for academic term parsing and exact-contract validation. The supported EnrollPro structures are trimester positions 1..3 and quarter positions 1..4; a requested index must exist in the exact school/year contract. No term is clamped, cycled, defaulted, relabelled, or dropped.
- Generation/review reads and exports accept explicit configured indices even when the current active term is unresolved. `termIndex=active` resolves only from persisted verified authority and fails closed when unavailable.
- Initial publication and published-revision creation validate source and changed entry terms against the exact contract inside their existing protected transaction boundaries. Public and privileged published-schedule filters preserve Q4 and revision-effective ordering.
- `/api/v1/runtime/context` projects exact ordered EnrollPro labels. Timetable state, toolbar, pivots, and active-term selection consume those labels through a bounded numeric term type and repair a stale filter on school/year/contract changes.
- Generation input snapshots use schema version 2 and require the derived-demand domain. A schema-v1 run is readable but resolves `UNKNOWN` with `SNAPSHOT_VERSION_MISMATCH`; historical rows are not rewritten.
