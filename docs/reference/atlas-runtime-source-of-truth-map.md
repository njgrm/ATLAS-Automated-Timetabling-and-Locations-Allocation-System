...
DEMAND-C01 canonical derived demand authority (`2026-09-11`, `REVIEW_REQUIRED`):
- Demand authority is now ONE deterministic read-only contract in `atlas-server/src/services/derived-demand.service.ts`, derived from (1) the sole active, non-archived EnrollPro school year and its verified ordered term structure, (2) active ATLAS section mirrors, and (3) ATLAS Subject scheduling metadata (`schedulingDisposition`, grade/program scope, rotation family + order, `minMinutesPerWeek`). `SchoolYearOffering`, `OfferingTermAssignment`, and `SchoolYearTermConfig` are NEVER selected as current-year demand truth.
- A timetable demand identity is `school/year/term/subject/section`; a Teaching Load demand identity is the unique `school/year/subject/section` pair across terms. `SCHEDULED_TEACHING` subjects occur in every ordered term; validated rotating-family members occur only in their ordered term. `REFERENCE_ONLY` subjects (including HG) create zero timetable lines and zero Teaching Load pairs. Grade/program scope is exact; missing or out-of-range/duplicate rotation metadata and an unavailable ordered term structure fail closed with typed blockers (never a silent cycle or fallback).
- One SHA-256 semantic revision binds school/year, the ordered terms, the active sections, and every semantic Subject field used by derivation. Counts/timestamps are diagnostic only; read-order permutations do not change it, and same-count semantic edits do. Persisted offering rows cannot change it.
- Consumers wired to the contract: `teaching-load-reconciliation.service.ts` (demand snapshot/preview and plan), `timetable-demand.service.ts` (insertion summary/preview), and `generation.service.ts` (canonical scheduler `demandOverride`, with `runHybridScheduler` honoring the override instead of calling `computeDemand()`). Each surfaces the same `derivedDemandRevision`; generation records it in the run summary. Preview/read paths are zero-write. `pre-generation-draft.service.ts` still uses the legacy catalog `computeDemand()` and is a known residual for the successor generation-readiness stream.

DEMAND-C01R derived-demand authority closure (`2026-09-11`, `REVIEW_REQUIRED`):
- Rotation authority is validated per active normalized grade/program scope: a family present in a scope must cover every configured ordered term `1..termCount`; a missing position is `ROTATION_INCOMPLETE`, duplicates/out-of-range are typed, and equal orders in disjoint scopes are not collisions.
- The scheduler term model supports term indices `1..4` (EnrollPro `TRIMESTER`/`QUARTERS`); a verified fourth term is never collapsed onto term 3. Rotation orders outside `1..MAX_SUPPORTED_TERMS` are reported missing, never clamped or aliased.
- The derived semantic revision (`DERIVED_DEMAND_V2`) binds ordered terms, active sections, Subject scheduling metadata (including `preferredRoomType` and `requiredFeatures`), and `periodLengthMinutes`. A period-length or room-semantics change changes the revision.
- Current-year term authority is the persisted `EnrollProSchoolYearMirror.termContractCache`, read exclusively through the supplied data-context client (no global Prisma, no live EnrollPro network call in a Serializable Teaching Load/publication transaction). Because PostgreSQL JSONB reorders object keys, the stored order-sensitive `semanticRevision` is not trusted; a canonical revision is recomputed. A missing/invalid snapshot fails closed with `TERM_STRUCTURE_UNAVAILABLE` and requires the explicit rollover/sync action.
- Generation/publication freshness includes a `derivedDemand` domain bound to that canonical revision; legacy `SchoolYearTermConfig`, `SchoolYearOffering`, and `OfferingTermAssignment` are removed from current-year authority. Publication revalidates derived authority inside its Serializable transaction and supports `QUARTERS`.
- Timetable summary/preview expose `derivedDemandRevision` and propagate a typed `DERIVED_DEMAND_BLOCKED` instead of an empty summary or `DEMAND_LINE_NOT_FOUND`. `toSchedulerDemandOverride` fails closed with a typed projection drift blocker and asserts exact projection parity.
- Live active year 9 (`2030-2031`) currently has no persisted verified term snapshot, so live derived demand, Timetable summary, and publication freshness fail closed until the operator runs the explicit rollover/sync. Zero writes.

DEMAND-C01R2 ordered-term consumer closure (`2026-09-11`, `REVIEW_REQUIRED`):
- One authoritative ordered-term model now lives in `atlas-server/src/services/academic-term.service.ts`: syntactic parsing accepts positive indices `1..4`, semantic validation rejects an index absent from the exact persisted contract (a trimester rejects 4; a quarter accepts 4), and the active term resolves only from the persisted verified EnrollPro contract, failing closed with `TERM_FILTER_NOT_READY` when it is unavailable or outside the contract. No term is clamped, cycled, defaulted, relabelled, or dropped.
- Generation/review reads (`getLatestRunViolations`, `getRunViolations`) validate a requested term against the verified structure; `termIndex=active` on the summary/class-program exports resolves through the persisted verified authority and fails closed. Workbook export no longer calls the live active-term adapter.
- Initial publication validates every run entry against the verified contract term count (`PUBLICATION_TERM_INDEX_OUTSIDE_CONTRACT`, zero writes); published-revision creation loads the verified contract inside its Serializable, advisory-locked transaction and rejects out-of-contract source, previous, and next term indices (`REVISION_TERM_INDEX_OUTSIDE_CONTRACT` / `REVISION_ENTRY_TERM_OUTSIDE_CONTRACT`, zero writes). Public/privileged published-schedule filters expose Q4 and resolve `active` from the persisted verified contract.
- `GET /api/v1/runtime/context` projects the exact ordered EnrollPro labels (`orderedTerms`, `termFormat`, `termCount`) from the persisted verified snapshot; the timetable toolbar/state and schedule pivots consume a bounded numeric term type with exact labels and `T1`/`T2`/... only as the fail-closed fallback. An invalid selected term is repaired to `all` on a school/year/contract change.
- The generation input snapshot is versioned honestly at `schemaVersion: 2` because `derivedDemand` is now a required freshness domain. A legacy schema-v1 run resolves to `UNKNOWN` / `SNAPSHOT_VERSION_MISMATCH` and can never compare as fresh; a current snapshot requires all six domains and stays deterministic under read-order permutations.
- Evidence: `atlas-server/src/__tests__/derived-demand-correction-c01r2.test.ts` (controls 1–10, including disposable quarterly PostgreSQL fixture for publication/revision/public reads) and `atlas-client/src/lib/__tests__/academic-term.test.ts` (control 7).
