...
- `Subject.schedulingDisposition` is the Subject catalog's scheduling-metadata authority: `SCHEDULED_TEACHING` creates projected timetable/Teaching Load demand, while `REFERENCE_ONLY` stays visible and creates neither. Existing rows default to scheduled teaching; migration backfill changes only normalized code `HG` to reference-only.
...
- Subject POST/PATCH may edit disposition but rejects EnrollPro-owned term count/format/identities/labels/dates and protected bootstrap metadata. Generation, allocation, timetable demand, Teaching Load, Dashboard, Year Setup, publication, and legacy Curriculum Requirements consumers remain unchanged in this cycle.
