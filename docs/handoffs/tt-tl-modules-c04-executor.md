...
| production-shape parity | producer: `previewDepartmentAuthority` (`department-authority.service.ts:331`) issuing `confirmationText`, plus the run-summary publication contract (`isRunPublishedStrict`) | consumer: `buildQualificationApplyPayload` (`TacticalSandboxDock.helpers.ts:448-467`) and `TimetableSimpleHeader.tsx:127,369` → `SimpleDriftBanner` | conservation: the server-issued confirmation is required verbatim for apply; a superseded run renders `isPublished:false` regardless of retained `publishedAt`/`publishedBy` | negative controls: `tt-tl-modules-helpers.test.ts:178-183`, `capability-override-mount.test.ts:203`, `tt-tl-modules-c04r1-behavior.test.ts:72-100` | **PASS** |
