...
**UX audit R1 integrated (2026-09-25):** `UX-AUDIT-SIZE-C01` candidate `fb245772` (QA `ACCEPT_READY` 6/6/0/0) — absolute `text-[14px]` / `text-[12px]` grid sizes replace the rem tokens that rendered 13.125 px at 390×844. Main's accepted contract is preserved: container-owned published read-only + disclosure, and `timetable-simple-daily-tasks` stays null — **F7 was deliberately rejected on main** ("daily repair tools remain in More so the header stays compact"). Client-only; merged-tree gates ux-audit-findings 21/21, client build + `git diff --check` clean. Live pixel rows (390×844 / 1366×768 rendered sizes, no global scrollbar) remain for the deployment acceptance; the shared runtime still serves the prior release.