...
| TT-TL-RUNTIME-ACCEPTANCE | Deploy integrated TT/TL source and perform read-only live diagnostics | `HIGH_APPROVAL_REQUIRED` | HIGH shared-runtime source deployment | `docs/prompts/tt-tl-runtime-acceptance-2026-09-12.md`; target `3d916b26`; current runtime `d44f29e0` | Requires explicit packet approval; no apply/generation/publication | Current Tailnet health is 200 on the supervised `d44f29e0` release; target source is descendant and combined source gates pass | Obtain approval for the prepared packet, then execute bounded stop/start and Tailnet read-only acceptance |
