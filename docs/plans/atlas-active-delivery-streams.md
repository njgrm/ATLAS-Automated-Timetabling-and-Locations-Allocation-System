...
| GEN-C02 | Use canonical term-aware derived demand and close grade-window/class-program-slot/hard-blocker gaps | `CORRECTION_REQUIRED` | MEDIUM source; HIGH generation | `integration/readiness-20260911`; `6f7b3c52...900ea7ff` | Actor-school gate absent; blocked readiness throws; trigger writes before canonical preflight and silently heals setup; assembly/TL-owner parity is split; grade/window authority is conflated; stakeholder matrix and mounted disposable-PG proof are missing; client wiring is owned by UX-C01 | Planner inspected the production range and reproduced year-9 `TERM_STRUCTURE_UNAVAILABLE` escaping as `DERIVED_DEMAND_BLOCKED`; live read confirmed zero grade-window rows while the trigger would create them; actual base is `6f7b3c52` | Execute the enlarged `docs/prompts/generation-canonical-readiness-genc02r-correction-2026-09-11.md` additively; do not touch UX-C01 client files or perform generation |
