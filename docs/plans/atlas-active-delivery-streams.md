...
| GEN-C02 | Use canonical term-aware derived demand and close grade-window/class-program-slot/hard-blocker gaps | `CORRECTION_REQUIRED` | MEDIUM source; HIGH generation | `integration/readiness-20260911`; `6f7b3c52...900ea7ff` | Missing stakeholder-shape matrix and mounted disposable-PostgreSQL zero-write proof; client wiring is owned by UX-C01 | Planner review confirmed the real trigger/readiness share shape assembly, but found no required grade/shift matrix and only an in-memory write recorder; actual base is `6f7b3c52` | Execute `docs/prompts/generation-canonical-readiness-genc02r-correction-2026-09-11.md` additively; do not touch UX-C01 client files |
