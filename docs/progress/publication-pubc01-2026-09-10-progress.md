...
## PUB-C01R2/R3 PostgreSQL concurrency correction (`REVIEW_REQUIRED`)

- Prior immutable candidate: `bcb3699152bb86e4456d156786bc727b69938d77`.
- R2 RED proof on disposable PostgreSQL reproduced SQLSTATE `42883`: Prisma raw numeric parameters were `INT8`, while the two-key advisory-lock overload requires `INT4`. Both publication services now use the same bound `pg_advisory_xact_lock($1::integer, $2::integer)` namespace and reject school/year identifiers outside positive Int32 range with their existing typed 4xx codes.
- R2 real overlap then exposed Prisma `P2034`: the second Serializable transaction used a stale snapshot and failed at the optimistic run update instead of replaying the first commit.
- R3 adds one shared, ESM-safe whole-transaction retry boundary. Only Prisma `P2034` or SQLSTATE `40001` retries, with three total attempts maximum. Every attempt starts a fresh Serializable transaction and reacquires the transaction-scoped advisory lock; application errors do not retry. Exhaustion returns `409 PUBLICATION_TRANSACTION_CONFLICT`. Notification handling remains after the retry boundary and executes only for a newly committed non-replay result.
- Real PostgreSQL matrix on `atlas_restore_drill_20260910_pubc01r3`: initial overlap observed two lock waiters and produced one revision / one audit / one notification attempt with original + replay; identical revision overlap produced one new revision / one audit / one notification attempt with original + replay; ordered non-identical overlap produced one advancing revision and typed `SOURCE_REVISION_STALE` with no rejected-request audit or notification.
- Database-backed zero-write guards passed for cross-school actor, inactive year, ambiguous active year, invalid term contract, stale input fingerprint, and causal effective date. Removing either SQL cast reproduced `42883`; overflow inputs reached no raw lock query; bound production lock events retained exactly two scope parameters.
- Runtime: built server started on isolated port `5128`, health returned 200, the published-revision route returned authentication status 401 rather than 404, rollover automation logged disabled with no start, and the port was free after shutdown. Port 5001 remained untouched.
- Cleanup: disposable database was created from `template0`, received the single repository migration (44 public tables), then was dropped and proved absent. The configured database remained `atlas_recovery_clean_rebuild_20260905` with one school and one migration.
- Focused gates: PostgreSQL concurrency suite PASS; publication-contract-readiness PASS; client published-revision 4/4 PASS; server/client `tsc --noEmit` PASS; server/client production builds PASS; worktree diff check clean. No live generation/publication, schema change, companion edit, merge, or push occurred.
