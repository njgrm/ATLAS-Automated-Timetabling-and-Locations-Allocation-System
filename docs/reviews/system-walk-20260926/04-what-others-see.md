...
| 3 | **SUPERSEDED — not reproduced; picking Luna changed the page and the URL. The real defect is narrower: changing the term clears the selected section (`PublicPublishedSchedule.tsx:523`; `docs/handoffs/planner-a2-timetable-browser-qa-handoff-2026-09-26.md`).** Original: **Public page: picking a section does nothing.** Clicking "Luna, GR7, 40 classes, Regular" left the detail pane on "Aguinaldo", twice. `/public/schedule` redirects to `?sectionId=143` (Aguinaldo). This is the page a parent uses. | P5 | HIGH |
...
| 5 | **SUPERSEDED — not reproduced; Help worked in the authenticated re-check (see the A2 handoff).** Original: **Help is broken.** The header "Help" button opened nothing; one click left the tab on an internal error page until `/` was reloaded. The guide exists only at `/timetabling/how-it-works` by typed URL. | P7 | HIGH (crash) |
