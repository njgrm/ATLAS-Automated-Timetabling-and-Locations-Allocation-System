...
replacement = '''type SubjectRowProps = {
	subject: Subject;
	assignment?: FacultyAssignmentDraft;
	sections: ExternalSection[];
	disabled: boolean;
...
if target in code:
    ...
    with open('src/pages/FacultyAssignments.tsx', 'w', encoding='utf-8') as f:
        f.write(code)
    print("Injected displaySections logic!")
else:
    print("Could not find exact block to inject displaySections.")
