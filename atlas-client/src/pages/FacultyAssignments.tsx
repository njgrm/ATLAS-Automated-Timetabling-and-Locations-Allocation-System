...
const COVERAGE_MODE_CONFIG: Record<CoverageMode, { label: string; description: string }> = {
	REAL_FACULTY_STANDARD: {
		label: 'Real Teacher Standard',
		description: 'Use current default policy load caps for real teacher coverage.',
	},
	REAL_FACULTY_HARD_CAP: {
		label: 'Real Teacher Hard Cap',
		description: 'Use real teachers only and stop at policy credited-load limits.',
	},
	REAL_FACULTY_THEN_TEACHER_X: {
		label: 'Real Teacher Then Teacher X',
		description: 'Fill with real teachers first, then create Teacher X placeholders for remaining gaps.',
	},
};
...
export default function FacultyAssignments() {
		...
		const handleApplyRealFacultyRecovery = useCallback(async () => {
			...
			try {
				...
				if (data.placeholderMovesApplied > 0) {
					toast.success(
						`Saved coverage reconciled: moved ${data.placeholderMovesApplied} row${data.placeholderMovesApplied === 1 ? '' : 's'} to real teachers.`,
					);
				} else {
				...
				toast.error(requestError?.response?.data?.message ?? 'Real-teacher recovery apply failed.');
			} finally {
	...
	const toggleTeacherXRoster = useCallback(() => {
		if (showTemporaryRoles) {
			...
			toast.info('Teacher X placeholders are now hidden from the teacher roster.');
			return;
		}
