          ...
          {(loadState.phase === 'ready' || loadState.phase === 'degraded') && summary && (
            <div className="space-y-3">
              <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground" data-testid="unassigned-insertion-stats">
                ...
                <Badge variant="secondary" className="h-7 px-2 text-xs">Ready to place {summary.insertionReadyLines}</Badge>
              ...
              <p className="text-xs text-muted-foreground">
                No timetable has been generated yet. This preview reflects the persisted curriculum and Teaching Load contract and never writes.
              </p>
                ...
                return (
                  <section key={group.reason} className="rounded-lg border border-border bg-muted/10 p-3" data-testid={`insertion-reason-group-${group.reason}`}>
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <Badge variant={group.reason === 'PLACEABLE' ? 'default' : 'secondary'} className="h-7 px-2 text-xs">
                        {group.count}
                      </Badge>
                    </div>
                    {group.guidance && (
                    ...
                    {group.reason === 'PLACEABLE' && (
                      <div className="mt-2 space-y-1">
                        <p className="text-xs text-muted-foreground">
                          {group.samples.length} sample line{group.samples.length === 1 ? '' : 's'} below — pick one to preview a candidate slot and room.
                        </p>
                        <ul className="space-y-1">
                          {group.samples.map((line) => (
                            <li key={line.demandKey} className="flex flex-wrap items-center justify-between gap-2 rounded-md border border-border bg-background px-2 py-1.5">
                              <div className="min-w-0">
                                <p className="truncate text-xs font-medium" data-testid="insertion-line-label">
                                  {line.subjectName} · {line.sectionName} · {line.termIdentity}
                                </p>
                                <p className="truncate text-[0.7rem] text-muted-foreground">
                                  {line.ownerFacultyName ?? 'No owner'} · {line.sessionsPerWeek} session{line.sessionsPerWeek === 1 ? '' : 's'}/week
                                </p>
                              </div>
                              <Button
                                type="button"
                                variant="outline"
                                size="sm"
                                className="h-11 min-w-28 px-3 text-xs"
                                disabled={previewingKey === line.demandKey}
                                onClick={() => void handlePreview(line.demandKey)}
                                data-testid={`insertion-preview-${line.demandKey}`}
                              >
        ...
        {preview && previewLine && (
          <div className="shrink-0 border-t px-4 py-3">
            <div className="flex items-center justify-between gap-2">
              ...
              <Badge variant={preview.state === 'PLACEABLE' ? 'default' : 'secondary'} className="h-7 shrink-0 px-2 text-xs" data-testid="insertion-preview-state">
                {preview.state === 'PLACEABLE' ? 'Ready to place' : preview.state}
              </Badge>
            </div>
            {preview.state === 'PLACEABLE' && preview.candidates.length > 0 && (
            ...
            {previewError && (
              <p className="mt-2 text-xs text-destructive" role="alert" data-testid="insertion-preview-error">
                {previewError}
              </p>
            )}
          </div>
        ...
        <DialogFooter className="shrink-0 border-t px-4 py-3">
          <div className="flex w-full flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <p className="max-w-md text-xs text-muted-foreground" data-testid="insertion-save-boundary-note">
              {preview ? save.detail : 'Preview a placeable line to review teacher, subject, section, term, day/time, and room.'}
            </p>
            <Button
              type="button"
              size="sm"
