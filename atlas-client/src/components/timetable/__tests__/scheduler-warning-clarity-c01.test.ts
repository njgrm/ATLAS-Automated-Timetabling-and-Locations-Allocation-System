...
test('warning affordance is keyboard discoverable and explains severity and publication blocking', () => {
	const grid = read('TimetableGrid.tsx');
	assert.match(grid, /TooltipTrigger asChild/);
	assert.match(grid, /aria-label=.*warning/i);
	assert.match(grid, /blocks saving and publishing|does not block saving or publishing/i);
