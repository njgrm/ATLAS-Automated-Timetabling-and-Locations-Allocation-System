	...
	return (
		<div className="rounded-xl border border-slate-100 bg-slate-50/70 p-3 shadow-sm" data-testid="subject-mobile-card">
			<div className="flex items-start justify-between gap-3">
				<div className="min-w-0">
					<div className="flex flex-wrap items-center gap-1.5">
						...
						{isReferenceOnly && <Badge variant="outline" className="h-4 px-1.5 text-[0.65rem] font-bold bg-slate-50 text-slate-700 border-slate-300 shadow-none">Reference only</Badge>}
					</div>
			...
			<div className="mt-2.5 space-y-1.5 text-xs">
				{isReferenceOnly ? (
					<p className="rounded-md border border-slate-200 bg-white px-2 py-1.5 font-semibold text-slate-700">Creates neither timetable demand nor Teaching Load.</p>
				) : null}
				...
				<div className="flex items-center justify-between gap-2">
					...
					{isReferenceOnly ? (
						<span className="font-semibold text-slate-700">Not applicable</span>
					) : isArchived ? (
			...
			<div className="mt-3 flex items-center gap-2 border-t border-slate-200/70 pt-3">
				{!isReferenceOnly ? (
					...
					<p className="flex-1 text-xs font-semibold text-muted-foreground">No coverage action</p>
				)}
				<DropdownMenu>
