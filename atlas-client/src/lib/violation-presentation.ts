...
export const VIOLATION_PRESENTATION: Record<ViolationCode, ViolationPresentation> = {
	...
	ROOM_FEATURE_MISMATCH: { title: 'Required room feature missing', meaning: 'The assigned room lacks equipment or features required by the subject.', action: 'Choose a room with the required features or correct the room inventory.' },
	...
	FACULTY_EXCESSIVE_TRAVEL_DISTANCE: { title: 'Legacy travel warning', meaning: 'An older run recorded a distance-based warning that current ATLAS no longer calculates.', action: 'Regenerate with the current policy before acting on this historical warning.' },
	FACULTY_FLOOR_TRANSITION: { title: 'Not enough time between floors', meaning: 'The teacher finishes on one floor and starts on another without enough time to move.', action: 'Add a gap or place one of the classes on the same floor.' },
