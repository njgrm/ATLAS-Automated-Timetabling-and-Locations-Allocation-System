...
export function useScheduleReviewWorkspaceState() {
	...
	useEffect(() => {
		if (!collaborationConnected || !collaborationRef.current || !selectedEntry || !runIdNumeric) return;
	...
	const handleCellDrop = useCallback(
		...
		[dragItem, previewEdit, commitEdit, stagePreGenDrop, centerView, draftBoard?.placements, draft?.entries, openRegularSwapPrompt, entityFilter, viewMode],
	);
	...
	const handleKbPlace = useCallback(
		...
		[kbSelectedSource, previewEdit, commitEdit, stagePreGenDrop, centerView, draftBoard?.placements, draft?.entries, openRegularSwapPrompt, entityFilter, viewMode],
	);
	...
	const gradeForSection = useCallback(
		(sectionId: number): number | null => {
			...
			const entry = draft?.entries.find((e: any) => e.sectionId === sectionId);
	...
	const handleGlobalDragEnd = useCallback((event: DragEndEvent) => {
		...
		dragItemRef.current = null;
		setDragItem(null);
	}, [draftBoard?.placements, focusPinnedPlacement, handleCellDrop, setPinnedRailDropActive, setPendingUnassignId, setShowUnassignConfirm, setUnassignDropActive]);
	...
	const workspaceContexts = useMemo(() => {
		if ((loading && !draft) || error) {
			return {
				leftRailContentContext: undefined as any,
				centerWorkspaceContext: undefined as any,
				rightPanelContext: undefined as any,
				headerContext: undefined as any,
				overlaysContext: undefined as any,
				dialogContext: undefined as any,
			};
		}
