...
// Use regex to locate SubjectRowProps up to const [openGrades...
const regex = /type SubjectRowProps = \{[\s\S]*?function SubjectRow\(\{[\s\S]*?\}\: SubjectRowProps\) \{[\s\n]*const \[openGrades, setOpenGrades\] = useState<Record<number, boolean>>\(\{\}\);\s*const groupedSections = useMemo\(\(\) => \{[\s\n]*const sectionGroups = new Map<number, ExternalSection\[\]>\(\);/;
...
function SubjectRow({
    ...
    console.log("Fix injected successfully with regex.");
} else {
    console.log("Could not find the target block using regex either.");
}
