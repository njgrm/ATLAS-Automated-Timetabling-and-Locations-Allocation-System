...
- **2026-07-20 timetable overhaul Iteration A:** Phase 0A + Phase 0 are **Antigravity GO**. `/timetable` now shows a source-truth notice for saved/stale school-year context and completed-run fallback behind newer failed runs; generated unassigned items with faculty but no `homeRoomId` now show `Needs room` / `Fix room first` instead of false `Place session`; generated occupied-slot swap now confirms with `Swap sessions` instead of `Apply repair`. New live gate `timetable-overhaul-iteration-a.spec.ts` passed `9/9` across desktop, mobile portrait, and mobile landscape. Supporting gates passed: client `tsc --noEmit`, client production build, `test:ux-guardrails` `26/26`, and `test:timetable-conflict` `10/10`. Overall overhaul remains **NO-GO** pending Iteration C external verification and Iteration D.
- **2026-07-20 timetable overhaul Iteration B:** Phase 1 + Phase 2 are **Antigravity GO**. Generated unassigned items with Teaching Load owners but missing home rooms now reach `Review generated placement` through click-to-place and drag-to-place; the review locks the teacher to Teaching Load and asks only for the room source before save. Generated drag now resolves the underlying timetable cell with viewport hit-testing, and manual edit commits return a real success/failure signal so placement and move flows no longer show false success after failed commits. Generated occupied-slot swaps remain on `Review occupied-slot swap` / `Swap sessions`. Supporting gates passed: client `tsc --noEmit`, client production build, `test:ux-guardrails` `26/26`, `test:timetable-conflict` `10/10`, and a live Tailnet browser matrix covering Iteration A, Iteration B, Phase 01, and Phase 05 passed `30/30` across desktop, mobile portrait, and mobile landscape. Overall overhaul remains **NO-GO** pending Antigravity verification for Iteration C and implementation of Iteration D.
- **2026-07-21 timetable overhaul Iteration B external verification:** Iteration B is now **Antigravity GO** based on the user's supplied live Tailnet verification. Antigravity reported `tsc`, UX guardrails, timetable conflict tests, production build, and a `30/30` Playwright matrix PASS, including click-to-place, drag-to-place, modern generated placement review, Teaching Load ownership constraints, and modern generated occupied-slot swap review.
- **2026-07-21 timetable overhaul Iteration C:** Phase 3 + Phase 3A + Phase 4 are **Codex GO, pending Antigravity review**. Shared setup pages now use compact command-band headers and compact content shells for `/sections`, `/subjects`, and `/faculty`; Teaching Load exposes the working area sooner by removing duplicate visual workflow prose from the first lane; timetable source/task guidance and command row are compressed while preserving Iteration A/B source, placement, drag, and swap behavior. Supporting gates passed: client `tsc --noEmit`, client production build, `test:ux-guardrails` `27/27`, `test:timetable-conflict` `10/10`, Iteration C browser gate `15/15`, and combined Iteration A/B/C live Tailnet regression `33/33` across desktop, mobile portrait, and mobile landscape. Overall overhaul remains **NO-GO** pending Antigravity verification for Iteration C and implementation of Iteration D.
- **2026-07-21 timetable overhaul Iteration C external verification:** Iteration C is now **Antigravity GO** based on the user's supplied live Tailnet validation. Antigravity reported `33/33` Playwright assertions PASS across desktop, mobile portrait, and mobile landscape, with compact headers, no global scrollbars, early useful content visibility, and no Iteration A/B regressions.
- **2026-07-21 timetable overhaul Iteration D:** Phase 5 + Phase 6 + Phase 7 technical work is **Antigravity GO for external performance re-test, pending moderated older-user evidence**. Generated placement, generated swap, draft placement, and draft swap now share one review/action-sheet pattern; selected timetable sessions expose a bottom selection strip with `Move timeslot` and `Details`; pointer-drag grid-wide guidance is batched outside React cell rerenders while preserving `Can place`, `Can swap`, `Blocked`, and `Warning` labels. Supporting gates passed: client `tsc --noEmit`, client production build, `test:ux-guardrails` `28/28`, `test:timetable-conflict` `10/10`, Iteration D browser gate `12/12`, Tailnet performance desktop `14/14`, Tailnet performance mobile portrait + mobile landscape `28/28`, and combined Iteration A/B/C/D live regression `45/45`. Antigravity later reported a valid NO-GO on mobile-portrait drag containment (`maxCellsPerCommitBatch=3`) plus compact panel warnings; Codex corrected pointer active-cell throttling, compact panel defaults/handles, and compact draft-source handoff, then reran `tsc`, UX guardrails `28/28`, conflict tests `10/10`, production build, Iteration D browser gate `12/12`, and full Tailnet performance matrix `42/42` with latest pointer drag `maxCellsPerCommitBatch=2`, `60.10 FPS`, zero long tasks, and zero header/left/right commits. Antigravity's re-test then passed the failing mobile-portrait performance matrix `14/14`, including React commit containment and mandatory gate verdict. Overall overhaul remains **NO-GO** only for final moderated older-user product validation.
- **2026-07-21 timetable default-layout redesign plan:** User validation confirms the prior overhaul is still visually too close to the old cockpit and remains overwhelming. A new dedicated stream is planned in `docs/phases/timetable-default-layout-redesign-plan-2026-07-21.md`: Iteration E introduces simple/advanced layout mode plus a slim status/task prompt; Iteration F replaces persistent default panels with task drawers; Iteration G reduces grid visual density and completes older-user accessibility; Iteration H handles AG/browser validation and moderated older-user testing. Default-layout redesign remains **NO-GO** until this stream is implemented and externally validated.
- **2026-07-21 timetable default-layout redesign Iterations E-H:** Default-layout redesign is **Codex GO, pending Antigravity external verification and moderated older-user validation**. `/timetable` now defaults to Simple view with a slim source/readiness/next-step header, visible `Filters`, reversible `Advanced view` / `Simple view`, and no persistent default rails. Focused task drawers now open only after choosing a task; Simple-mode grid cards have larger targets and reduced section microtext while preserving grid-wide `Can place`, `Can swap`, `Blocked`, `Warning`, and `Occupied` guidance. Supporting gates passed: client `tsc --noEmit`, `test:ux-guardrails` `29/29`, `test:timetable-conflict` `10/10`, client production build, default-layout Tailnet browser gate `9/9`, and full Tailnet performance matrix `42/42`. Closure report: `docs/verification/timetable-default-layout-redesign-closure-2026-07-21.md`; AG prompts: `docs/prompts/antigravity-timetable-default-layout-iteration-e-validation-2026-07-21.md` through Iteration H.
- **2026-07-27 timetable finalization caveats plan:** Remaining closure work is planned in `docs/phases/timetable-finalization-caveats-plan-2026-07-27.md`. The plan separates stale A-D Playwright contract failures from real product failures, keeps the new Simple/Advanced layout as the canonical timetable contract, adds a Tailnet runtime preflight gate, identifies `/sections` compact-header density as a broader cross-page caveat, and defines final technical plus moderated older-user validation gates before Product GO.
- **2026-07-27 timetable finalization Iterations I-M:** Iterations I-L are **Technical GO**. A-D and C Playwright specs now follow the Simple-default/Advanced-opt-in contract; the compact setup shell gate passes `15/15`; Tailnet preflight is documented and automated; static gates passed (`tsc`, UX guardrails `30/30`, timetable conflict `10/10`, production build); final browser layout/workflow matrix passed `57/57`; full Tailnet performance matrix passed `42/42`. Iteration M is prepared with an executable moderated older-user validation script, but **Product GO remains pending** real participant evidence or stakeholder acceptance to defer. Closure report: `docs/verification/timetable-final-technical-closure-2026-07-27.md`; moderated script: `docs/verification/timetable-moderated-older-user-validation-2026-07-27.md`.
- **2026-07-28 setup-first UI/UX Iterations 0-2:** Iterations 0-2 are **Codex GO, pending optional Antigravity review**. New plan and baseline audit were added at `docs/phases/setup-first-uiux-overhaul-plan-2026-07-28.md` and `docs/analysis/setup-first-uiux-baseline-audit-2026-07-28.md`. Shared setup headers now keep source truth visible without hover through `admin-source-truth-summary`, Teaching Load exposes `teaching-load-source-truth-summary`, and the shared setup shell spacing was tightened after the first live gate found `/sections` at `155px` against the desktop `150px` cap. Supporting gates passed: client `tsc --noEmit`, `test:ux-guardrails` `30/30`, `test:timetable-conflict` `10/10`, client production build, Tailnet preflight `1/1`, new setup-first browser gate `15/15`, and existing Iteration C compact-shell regression `15/15` across desktop, mobile portrait, and mobile landscape.
- **2026-07-28 setup-first UI/UX Iterations 3-4:** Iterations 3-4 are **Codex GO, pending optional Antigravity review**. Teaching Load now opens with a compact next-task guide inside the content shell, advanced Teaching Load filters are disclosed behind `More filters`, mobile inspector tap interception is fixed, setup pages expose a shared `More filters` toolbar, subject program scopes are summarized into one tooltip-backed badge, and section home-room helper text is shorter. Supporting gates passed: client `tsc --noEmit`, `test:ux-guardrails` `32/32`, `test:timetable-conflict` `10/10`, client production build, new Iteration 3-4 Tailnet browser gate `15/15`, and Iteration 0-2 + timetable compactness regression `30/30` across desktop, mobile portrait, and mobile landscape.
- **2026-07-28 setup-first UI/UX Iterations 5-6:** Iterations 5-6 are **Codex GO, pending optional Antigravity review**. Campus/Rooms now leads with a plain `Room readiness` list and explicit `Ready`, `Needs capacity`, `Needs room type`, `Needs section`, and `Unavailable` labels; map/editor surfaces are lazy-loaded behind Suspense fallbacks. Dashboard readiness now covers Sections, Subjects, Teachers, Teaching Load, Rooms, Timetable, and Publish readiness with direct repair links and a visible source-state badge. Supporting gates passed: `test:ux-guardrails` `32/32`, `test:timetable-conflict` `10/10`, `tsc --noEmit`, client production build, new Iteration 5-6 Tailnet browser gate `6/6`, and combined Iterations 0-4 + 5-6 + timetable compactness regression `51/51` across desktop, mobile portrait, and mobile landscape. See `docs/verification/evidence-log.md` and `docs/phases/setup-first-uiux-overhaul-plan-2026-07-28.md`.
- **2026-07-28 setup-first UI/UX Iterations 7-8:** Iterations 7-8 are **Technical GO, pending optional Antigravity review and moderated older-user evidence**. Added final route/mode smoke coverage across `/`, `/sections`, `/subjects`, `/teachers`, `/teaching-load`, `/map`, and `/timetable`; fixed a Dashboard mobile-landscape loading-shell page scrollbar by moving scrolling into a local content region. Supporting gates passed: `tsc --noEmit`, `test:ux-guardrails` `32/32`, `test:timetable-conflict` `10/10`, client production build, Tailnet preflight `1/1`, Iteration 7-8 browser gate `6/6`, and combined setup-first/timetable regression `57/57` across desktop, mobile portrait, and mobile landscape. Closure report: `docs/verification/setup-first-uiux-iterations-7-8-closure-2026-07-28.md`.
- **2026-07-29 older-user session remediation Phase 4:** Phase 4 is **Technical GO, pending Phase 5 moderated older-user evidence**. Generated-unassigned virtualized queues now support CDP/browser-proxy touch scrolling on mobile portrait and mobile landscape without global page scroll; queue touch artifacts advanced `scrollTop 0 -> 176` on both mobile profiles. Coarse-pointer generated-unassigned cards prioritize scroll/tap by disabling drag listeners while desktop drag remains available. Supporting gates passed: focused Phase 4 browser gate `9/9`, Phase 0-4 regression matrix `45/45`, older-user T01-T12 browser validation `3/3`, `tsc --noEmit`, `test:ux-guardrails` `35/35`, `test:timetable-conflict` `10/10`, and client production build. Closure report: `docs/verification/older-user-session-remediation-phase-4-closure-2026-07-29.md`.
- **2026-07-29 older-user session remediation Phase 5:** Phase 5 is **GO WITH LIMITATION**. Real Scheduler Officer sessions were unavailable, so Codex executed a simulated five-participant validation by user instruction using the shared T01-T12 protocol, browser-proxy evidence, and static/build gates. Browser proxy T01-T12 passed `3/3` viewport sessions; Phase 4 touch/reflow passed `9/9`; supporting gates passed: `tsc --noEmit`, `test:ux-guardrails` `35/35`, `test:timetable-conflict` `10/10`, and client production build. Product GO is not claimed unless the stakeholder accepts simulated evidence as sufficient. Closure report: `docs/verification/older-user-session-validation-final-simulated-2026-07-29.md`.
