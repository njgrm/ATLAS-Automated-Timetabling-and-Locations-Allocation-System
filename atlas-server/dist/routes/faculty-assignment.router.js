...
import { autoFill, COVERAGE_MODES, previewOrApplyTeachingLoadSplitBrainReconcile, } from '../services/teaching-load-automation.service.js';
...
const router = Router();
function parseCoverageMode(value) {
    if (value == null) {
        return null;
    }
...
function parsePositiveQueryInteger(value) {
...
function parseSummaryListOptions(req) {
...
router.get('/summary', authenticateWithSystemToken, requirePrivilegedRole, async (req, res, next) => {
    try {
        ...
        const summary = await assignmentService.getAssignmentSummary(schoolId, schoolYearId, upstreamAuthToken, listRequested ? listOptions : undefined);
...
// Body: { schoolId: number, schoolYearId: number, previewOnly?: boolean, confirmApply?: boolean }
router.post('/integrity/reconcile-split-brain', authenticate, requirePrivilegedRole, async (req, res, next) => {
    try {
        const schoolId = Number(req.body.schoolId);
        const schoolYearId = Number(req.body.schoolYearId);
        const previewOnly = req.body.previewOnly !== false;
...
router.get('/capability-overrides', authenticate, requirePrivilegedRole, async (req, res, next) => {
    try {
        const schoolId = Number(req.query.schoolId);
        const schoolYearId = Number(req.query.schoolYearId);
        if (!schoolId || Number.isNaN(schoolId)) {
...
router.put('/capability-overrides', authenticate, requirePrivilegedRole, async (req, res, next) => {
    try {
        const schoolId = Number(req.body.schoolId);
        const schoolYearId = Number(req.body.schoolYearId);
...
router.delete('/capability-overrides', authenticate, requirePrivilegedRole, async (req, res, next) => {
    try {
        const schoolId = Number(req.body.schoolId);
        const schoolYearId = Number(req.body.schoolYearId);
        const facultyId = Number(req.body.facultyId);
...
router.post('/suggestion-proposals', authenticate, requirePrivilegedRole, async (req, res, next) => {
    try {
        const schoolId = Number(req.body.schoolId);
        const schoolYearId = Number(req.body.schoolYearId);
...
router.post('/suggestion-proposals/:proposalId/apply', authenticate, requirePrivilegedRole, async (req, res, next) => {
    try {
...
router.post('/suggestion-proposals/:proposalId/cancel', authenticate, requirePrivilegedRole, async (req, res, next) => {
    try {
        const proposalId = Number(req.params.proposalId);
        if (!proposalId || Number.isNaN(proposalId)) {
            res.status(400).json({ code: 'INVALID_PARAM', message: 'proposalId must be a valid number.' });
