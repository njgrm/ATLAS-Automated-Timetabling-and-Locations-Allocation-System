...
router.get('/:schoolId/:schoolYearId/runs/performance-fixture-source', authenticate, async (req, res, next) => {
    try {
        const role = req.user?.role;
        if (!role || !PRIVILEGED_ROLES.has(role)) {
...
router.post('/:schoolId/:schoolYearId/runs/:runId/performance-fixture', authenticate, async (req, res, next) => {
    try {
        const role = req.user?.role;
        if (!role || !PRIVILEGED_ROLES.has(role)) {
...
router.delete('/:schoolId/:schoolYearId/runs/:runId/performance-fixture', authenticate, async (req, res, next) => {
    try {
        const role = req.user?.role;
        if (!role || !PRIVILEGED_ROLES.has(role)) {
...
router.post('/:schoolId/:schoolYearId/runs/:runId/publish', authenticate, async (req, res, next) => {
    try {
        const role = req.user?.role;
        if (!role || !PRIVILEGED_ROLES.has(role)) {
...
router.get('/:schoolId/:schoolYearId/runs/latest/timetable', authenticate, async (req, res, next) => {
    try {
        const role = req.user?.role;
        if (!role || !PRIVILEGED_ROLES.has(role)) {
...
router.get('/:schoolId/:schoolYearId/runs/:runId/timetable', authenticate, async (req, res, next) => {
    try {
        const role = req.user?.role;
        if (!role || !PRIVILEGED_ROLES.has(role)) {
            res.status(403).json({ code: 'FORBIDDEN', message: 'Only admin, officer, or SYSTEM_ADMIN can view timetable entries.' });
...
router.get('/:schoolId/:schoolYearId/runs/:runId/export/summary-teacher-schedule.xlsx', authenticate, async (req, res, next) => {
    try {
        const role = req.user?.role;
        if (!role || !PRIVILEGED_ROLES.has(role)) {
...
router.get('/:schoolId/:schoolYearId/runs/:runId/export/class-program.xlsx', authenticate, async (req, res, next) => {
    try {
        const role = req.user?.role;
        if (!role || !PRIVILEGED_ROLES.has(role)) {
            res.status(403).json({ code: 'FORBIDDEN', message: 'Only admin, officer, or SYSTEM_ADMIN can export workbooks.' });
