...
const VALID_REVIEW_DECISIONS = new Set(['APPROVED', 'REJECTED', 'NEEDS_FOLLOW_UP']);
...
router.get('/:schoolId/:schoolYearId/latest/me', authenticate, async (req, res, next) => {
    try {
        const schoolId = positiveInt(req.params.schoolId, 'schoolId');
        if (typeof schoolId === 'string') {
            res.status(400).json({ code: 'INVALID_PARAM', message: schoolId });
        ...
        const result = await roomPreferenceService.getLatestFacultyRoomPreferenceState(schoolId, schoolYearId, identity.faculty.id);
...
router.post('/:schoolId/:schoolYearId/runs/:runId/faculty/:facultyId/entries/:entryId/preview', authenticate, async (req, res, next) => {
    try {
        const scope = parseScope(req, res);
        if (!scope)
            return;
...
router.post('/:schoolId/:schoolYearId/runs/:runId/faculty/:facultyId/sync', authenticate, async (req, res, next) => {
    try {
        const scope = parseScope(req, res);
        if (!scope)
            return;
