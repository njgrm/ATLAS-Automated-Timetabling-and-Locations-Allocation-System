...
const router = Router();
const VALID_PROGRAM_TYPES = new Set(['REGULAR', 'STE', 'SPS', 'SPA', 'OTHER']);
...
router.put('/:id/subjects', authenticate, requirePrivilegedRole, async (req, res, next) => {
    try {
        const id = Number(req.params.id);
        if (Number.isNaN(id)) {
            res.status(400).json({ code: 'INVALID_PARAM', message: 'id must be a number.' });
...
//# sourceMappingURL=class-template.router.js.map