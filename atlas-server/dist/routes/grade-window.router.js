...
router.get('/:schoolId/:schoolYearId/grade-windows', authenticate, async (req, res, next) => {
    try {
...
router.put('/:schoolId/:schoolYearId/grade-windows', authenticate, async (req, res, next) => {
    try {
        const role = req.user?.role;
        if (!role || !PRIVILEGED_ROLES.has(role)) {
...
router.delete('/:schoolId/:schoolYearId/grade-windows/:gradeLevel', authenticate, async (req, res, next) => {
    try {
        const role = req.user?.role;
        if (!role || !PRIVILEGED_ROLES.has(role)) {
        ...
        if (typeof gradeLevel === 'string') {
            res.status(400).json({ code: 'INVALID_PARAM', message: gradeLevel });
            return;
        }
...
//# sourceMappingURL=grade-window.router.js.map