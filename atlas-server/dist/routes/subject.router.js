...
router.post('/', authenticate, requirePrivilegedRole, async (req, res, next) => {
    try {
        const { schoolId, code, name, minMinutesPerWeek, preferredRoomType, gradeLevels, isSeedable, interSectionEnabled, interSectionGradeLevels, modularGroupId, modularOrder, termGroupId, termCount, programScopes, allowedSpecializations, requiredFeatures, allowedOwnerDepartments, isActive, ownerDepartment, qualificationPriority, rotationFamily, outputLabel, isSystemManaged, } = req.body;
...
router.post('/:id/reactivate', authenticate, requirePrivilegedRole, async (req, res, next) => {
    try {
        const id = Number(req.params.id);
        if (Number.isNaN(id)) {
            res.status(400).json({ code: 'INVALID_PARAM', message: 'id must be a number.' });
