...
async function readSseUntil(baseUrl, path, token, matcher, timeoutMs) {
...
async function run() {
    ...
    try {
        ...
        const officerSsePromise = readSseUntil(baseUrl, sseEventsPath, officerToken, (text) => text.includes('PREFERENCE_SUBMITTED'), 6000);
        // Save draft first
        const draftRes = await requestJson(baseUrl, `/preferences/${schoolId}/${syId}/faculty/${facultyId}/draft`, {
            method: 'PUT',
            headers: facultyAuth,
            body: JSON.stringify({
        ...
        const draftVersion = draftRes.json?.preference?.version ?? 1;
        // Submit
        const submitRes = await requestJson(baseUrl, `/preferences/${schoolId}/${syId}/faculty/${facultyId}/submit`, {
            method: 'POST',
            headers: facultyAuth,
            body: JSON.stringify({
                notes: 'SSE bilateral test',
        ...
        assert(officerSseBuffer?.includes('PREFERENCE_SUBMITTED') ?? false, 'Officer SSE event type is PREFERENCE_SUBMITTED');
        ...
        const facultySsePromise = readSseUntil(baseUrl, sseEventsPath, facultyToken, (text) => text.includes('PREFERENCE_REVIEWED'), 6000);
        // Get the submitted preference ID
        const prefDetail = await requestJson(baseUrl, `/preferences/${schoolId}/${syId}/faculty/${facultyId}/detail`, { headers: { Authorization: `Bearer ${officerToken}` } });
        assertEqual(prefDetail.status, 200, 'Officer can fetch preference detail');
        const prefId = prefDetail.json?.preference?.id;
        assert(!!prefId, 'Preference ID is returned');
        // Officer reviews
        const reviewRes = await requestJson(baseUrl, `/preferences/${schoolId}/${syId}/review/${prefId}`, {
            method: 'PATCH',
            headers: officerAuth,
        ...
        assert(facultySseBuffer?.includes('PREFERENCE_REVIEWED') ?? false, 'Faculty SSE event type is PREFERENCE_REVIEWED');
    }
    finally {
...
//# sourceMappingURL=preference-sse-bilateral.test.js.map