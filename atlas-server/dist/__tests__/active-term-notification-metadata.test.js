...
import { publishNotificationEvent, subscribeNotificationEvents, disposeNotificationEventBridges, initializeNotificationEventBridges, } from '../services/notification-events.service.js';
...
test('publishNotificationEvent passes through metadata for generation completion', () => {
    ...
    check(event?.metadata?.termCounts?.term1 === 50, 'termCounts.term1 = 50');
    check(event?.metadata?.termCounts?.term2 === 30, 'termCounts.term2 = 30');
    check(event?.metadata?.termCounts?.term3 === 25, 'termCounts.term3 = 25');
    teardown(stop);
});
...
test('publishPublishedScheduleEvent passes through metadata for schedule revision', () => {
    ...
    check(Array.isArray(event?.metadata?.affectedTermIndices), 'affectedTermIndices is array');
    check(event?.metadata?.affectedTermIndices?.includes(1), 'affectedTermIndices includes 1');
    check(event?.metadata?.affectedTermIndices?.includes(2), 'affectedTermIndices includes 2');
    teardown(stop);
});
...
test('publishTimetableEvent passes through metadata for single edit', () => {
    ...
    check(event?.metadata?.termIndex === 1, 'termIndex = 1');
    teardown(stop);
});
...
test('publishTimetableEvent passes through metadata for batch edit', () => {
    ...
    check(Array.isArray(event?.metadata?.affectedTermIndices), 'affectedTermIndices is array');
    check(event?.metadata?.affectedTermIndices?.includes(1), 'affectedTermIndices includes 1');
    check(event?.metadata?.affectedTermIndices?.includes(3), 'affectedTermIndices includes 3');
    teardown(stop);
});
...
test('publishTimetableEvent passes through metadata for revert', () => {
    ...
    check(event?.metadata?.termIndex === 2, 'termIndex = 2');
    teardown(stop);
});
...
test('publishTimetableEvent works without term metadata (backward compatible)', () => {
    ...
    const event = events.at(-1);
    check(event?.type === 'TIMETABLE_EDIT_COMMITTED', 'Event type is TIMETABLE_EDIT_COMMITTED');
    check(event?.metadata?.termIndex === undefined, 'termIndex is undefined');
    check(event?.metadata?.affectedTermIndices === undefined, 'affectedTermIndices is undefined');
    teardown(stop);
});
...
test('publishTimetableEvent passes through metadata for swap', () => {
    ...
    check(event?.type === 'TIMETABLE_EDIT_COMMITTED', 'Event type is TIMETABLE_EDIT_COMMITTED');
    check(Array.isArray(event?.metadata?.affectedTermIndices), 'affectedTermIndices is array');
    check(event?.metadata?.affectedTermIndices?.length === 1, 'affectedTermIndices has 1 entry');
    check(event?.metadata?.affectedTermIndices?.includes(1), 'affectedTermIndices includes term 1');
    teardown(stop);
});
