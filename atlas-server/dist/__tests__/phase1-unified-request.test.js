...
async function run() {
    ...
    try {
        ...
        const token = facultyLogin.json?.token;
        ...
        const sourceEntryId = latestState.json?.entries?.[0]?.entryId;
        ...
        const swapPreview = await requestJson(baseUrl, `/room-preferences/${schoolId}/${schoolYearId}/runs/${run.id}/faculty/${facultyAccounts[0].facultyId}/entries/${sourceEntryId}/preview`, {
            ...
            body: JSON.stringify({
                ...
                targetEndTime: '11:00',
                targetEntryId: run.draftEntries[1].entryId,
                expectedRunVersion: run.version,
            }),
        });
...
//# sourceMappingURL=phase1-unified-request.test.js.map