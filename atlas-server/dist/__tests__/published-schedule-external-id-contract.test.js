...
async function run() {
    ...
    try {
        ...
        const entries = historicalPublished.draftEntries;
        ...
        const sampleEntry = payloadEntries.find((e) => e.faculty?.atlasId === testFacultyId && e.section?.externalId === testSectionId)
            ?? payloadEntries.find((e) => e.faculty?.atlasId === testFacultyId)
            ?? payloadEntries[0];
        if (sampleEntry) {
        ...
        if (facultyMirror) {
            ...
            const allMatchExternalId = extEntries.every((e) => e.faculty?.externalId === facultyMirror?.externalId);
        ...
        const unassignedEntry = payloadEntries.find((e) => e.faculty?.id == null);
        if (unassignedEntry) {
...
run();
//# sourceMappingURL=published-schedule-external-id-contract.test.js.map