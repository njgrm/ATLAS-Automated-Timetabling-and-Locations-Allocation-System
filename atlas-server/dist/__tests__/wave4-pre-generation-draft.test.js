...
section('Wave 4.5c — source guards for atomic swap and soft-warning policy');
{
    ...
    assert(clientMutationsSource.includes('excludePlacementIds: [displaced.id]'), 'Client swap dialog simulates displaced-slot previews before swap confirmation');
    assert(clientMutationsSource.includes('scopePreviewToCandidate(displacedPreviewRaw'), 'Client swap dialog scopes the displaced leg preview independently');
...
//# sourceMappingURL=wave4-pre-generation-draft.test.js.map