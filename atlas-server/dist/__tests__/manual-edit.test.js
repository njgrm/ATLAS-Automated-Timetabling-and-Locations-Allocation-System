...
const rooms = [
    { id: 1, type: 'CLASSROOM', capacity: null },
    { id: 2, type: 'CLASSROOM', capacity: null },
    { id: 3, type: 'LABORATORY', capacity: null },
];
const subjects = [
    { id: 1, preferredRoomType: 'CLASSROOM' },
    { id: 2, preferredRoomType: 'CLASSROOM' },
];
...
function applyProposal(entries, unassigned, proposal) {
    ...
    if (proposal.editType === 'PLACE_UNASSIGNED') {
        const uIdx = newUnassigned.findIndex((u) => u.sectionId === proposal.sectionId && u.subjectId === proposal.subjectId &&
...
section('Place unassigned: Not found');
{
    try {
        ...
        assert(e.message.includes('not found'), 'Throws when unassigned not found');
    }
}
section('Move entry: Entry not found');
{
    try {
        ...
        assert(e.message.includes('not found'), 'Throws when entry not found');
    }
}
...
if (failCount > 0)
    process.exit(1);
//# sourceMappingURL=manual-edit.test.js.map