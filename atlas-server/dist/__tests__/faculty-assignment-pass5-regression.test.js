import { __testResolveOwnedCurrentYearSectionScope, buildRotationTermBreakdown, buildDuplicateOwnershipBlockingResult, computeTeachingLoadMinutes, detectDuplicateOwnershipTuples, resolveAssignmentSpecializationIdentity, } from '../services/faculty-assignment.service.js';
import { __testAggregateSplitBrainCoverageTotals, __testComputeCreditedCapacityMinutes, __testEstimateCapacityLaneDeltaMinutes, __testRankCoverageCandidates, __testResolveSplitBrainQuarantine, } from '../services/teaching-load-automation.service.js';
...
section('Hard-cap delta respects rotational peak terms');
{
    ...
    const nonPeakDelta = __testEstimateCapacityLaneDeltaMinutes(baselineLanes, 'family:SCIENCE:term:3:701', 225);
    assertEqual(nonPeakDelta, 0, 'Adding a non-peak term lane does not consume additional credited hard-cap minutes');
    const peakGrowthDelta = __testEstimateCapacityLaneDeltaMinutes(baselineLanes, 'family:SCIENCE:term:1:703', 225);
