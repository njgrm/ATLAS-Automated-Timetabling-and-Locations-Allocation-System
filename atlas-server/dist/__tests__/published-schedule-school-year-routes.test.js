...
async function run() {
    ...
    try {
        ...
        if (historicalPublished) {
            ...
            const entries = (histEntries?.draftEntries ?? []);
            if (entries.length > 0) {
                const testSectionId = entries[0].sectionId;
                const filteredResp = await requestJson(`/api/v1/schools/${DEFAULT_SCHOOL_ID}/school-years/${historicalPublished.schoolYearId}/schedules/published/sections/${testSectionId}`);
                ...
                const hasOtherYearEntries = returnedEntries.some((e) => {
...
run();
//# sourceMappingURL=published-schedule-school-year-routes.test.js.map