...
function extractGradeNumber(gradeLevelName) {
    // "Grade 7" -> 7, "Grade 10" -> 10
    const match = gradeLevelName.match(/(\d+)/);
    if (match) {
        const n = parseInt(match[1], 10);
        if (VALID_GRADES.has(n))
            return n;
    }
    return 0;
}
function buildingMatchScore(sectionGrade, buildingGradeScope) {
    // 0 = no match (cross-grade), 1 = any-grade building, 2 = exact grade match
    if (buildingGradeScope.length === 0)
        return 1; // any-grade
    if (buildingGradeScope.includes(sectionGrade))
        return 2; // exact match
    return 0; // no match
}
function sortKey(section, room, matchScore) {
    return {
...
export async function computeAutoAssign(options) {
    ...
    const sortedSections = [...sectionsToProcess].sort((a, b) => a.gradeLevelId - b.gradeLevelId || a.name.localeCompare(b.name));
    ...
    for (const section of sortedSections) {
        ...
        const eligible = rooms
            ...
            if (entry.matchScore === 2)
                return true; // exact grade match
            if (entry.matchScore === 1)
                return true; // any-grade building
...
//# sourceMappingURL=home-room-auto-assign.service.js.map