...
const VIEW_MODES = new Set(['FACULTY_ACTIVE_DRAFT', 'SCHEDULER_REVIEW', 'SCHEDULER_QUEUE']);
function normalizeAuthPayload(decoded) {
    return {
        ...decoded,
...
function normalizeViewMode(input, fallback) {
    if (typeof input !== 'string')
        return fallback;
    if (!VIEW_MODES.has(input))
        return fallback;
    return input;
}
...
export function registerRoomPreferenceCollaborationSocket(server, options) {
    ...
    function snapshotForChannel(channel) {
    ...
    function broadcastToChannel(channel, payload, exceptConnectionId) {
        for (const state of sockets.values()) {
            if (!state.joined)
                continue;
            if (!sameChannel(state.joined, channel))
                continue;
            if (exceptConnectionId && state.connectionId === exceptConnectionId)
                continue;
            safeSend(state.ws, payload);
        }
    }
    function closeWithReason(state, code, message) {
    ...
    async function canJoinChannel(auth, channel) {
        ...
        if (!run) {
            return { ok: false, code: 'RUN_NOT_FOUND', message: 'Run was not found in this school scope.' };
        }
...
//# sourceMappingURL=room-preference-collaboration.service.js.map