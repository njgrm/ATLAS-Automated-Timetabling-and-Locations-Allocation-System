...
function asScheduledEntry(input, entryId, demandItem) {
    return {
        entryId,
...
function placementToScheduledEntry(placement, demandItem) {
    return asScheduledEntry({
        ...
        cohortCode: placement.cohortCode,
        notes: placement.notes,
    }, `draft-${placement.id}`, demandItem);
}
function placementToInput(placement) {
...
function buildValidatorCtx(schoolId, schoolYearId, entries, ctx) {
    return {
        schoolId,
        schoolYearId,
        runId: 0,
        ...
        constraintConfig: {
            ...
            ...(ctx.policyRecord.constraintConfig ?? {}),
        },
    };
}
function validateInputOrThrow(input, ctx) {
...
function buildExistingEntries(ctx, excludedPlacementIds) {
...
async function buildBoardStateFromContext(schoolId, schoolYearId, ctx) {
    ...
    return {
        ...
        filters: {
            ...
            departments: [...new Set(ctx.facultyMirrors.map((faculty) => faculty.department).filter((department) => Boolean(department)))].sort((left, right) => left.localeCompare(right)),
...
function getDraftPlacementOrThrow(ctx, placementId, expectedVersion) {
...
function buildSwapPreview(ctx, sourcePlacement, targetPlacement) {
            ...
            data: [
                {
                    ...
                    actorId,
                    actionType: 'SWAP',
                    beforePayload: sourcePlacement,
                    afterPayload: nextSource,
                },
                {
                    ...
                    actorId,
                    actionType: 'SWAP',
                    beforePayload: targetPlacement,
                    afterPayload: nextTarget,
                },
            ],
        });
    ...
    await prisma.$transaction([
        ...
        prisma.lockedSessionAction.create({
            data: {
                ...
                beforePayload: draftPlacements,
                afterPayload: { archivedCount: draftPlacements.length },
            },
        }),
        prisma.auditLog.create({
            data: {
                ...
                metadata: { archivedCount: draftPlacements.length },
            },
        }),
    ]);
    return listDraftBoardState(schoolId, schoolYearId, authToken);
...
export async function undoLastPlacement(schoolId, schoolYearId, actorId, authToken) {
    ...
    await prisma.$transaction(async (tx) => {
        if (action.actionType === 'CREATE' && action.lockId != null) {
            ...
            const before = action.beforePayload;
            if (!before)
                throw err(500, 'UNDO_STATE_INVALID', 'Missing previous state for placement update undo.');
            ...
            const beforeList = Array.isArray(action.beforePayload) ? action.beforePayload : [];
        ...
        await tx.lockedSessionAction.create({
            data: {
                ...
                actorId,
                actionType: 'UNDO',
                beforePayload: action,
                afterPayload: { revertedActionId: action.id },
            },
        });
        await tx.auditLog.create({
            data: {
...
export async function markPlacementsLockedForRun(schoolId, schoolYearId, runId, placementIds) {
...
export async function archivePlacementsForRun(runId, schoolId, schoolYearId) {
    await prisma.lockedSession.updateMany({
        where: { schoolId, schoolYearId, lockedRunId: runId },
        data: { status: 'ARCHIVED', version: { increment: 1 } },
    });
}
//# sourceMappingURL=pre-generation-draft.service.js.map