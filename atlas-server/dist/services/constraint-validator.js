...
function isSameCohortGroup(left, right) {
...
export function validateHardConstraints(ctx) {
    ...
    const qualifiedSet = new Set(ctx.facultySubjects.flatMap((fs) => fs.sectionIds.map((sectionId) => `${fs.facultyId}:${fs.subjectId}:${sectionId}`)));
    ...
    if (ctx.travelPolicy?.enableTravelWellbeingChecks && ctx.buildings && ctx.roomBuildings) {
        ...
        for (const [key, dayEntries] of byFacDay) {
            ...
            for (let i = 1; i < sorted.length; i++) {
                ...
                if (fromBldId == null || toBldId == null)
                    continue;
    ...
    if (cc) {
        ...
        for (const v of violations) {
            ...
            if (!override.enabled && v.severity === 'SOFT')
                continue;
            // Promote to HARD if treatAsHard and currently SOFT
            const severity = (override.treatAsHard && v.severity === 'SOFT') ? 'HARD' : v.severity;
