...
function describePlacementScope(item) {
...
/* ─── Core function ─── */
export async function getFixSuggestions(schoolId, schoolYearId, runId, item) {
    ...
    switch (item.reason) {
        case 'NO_QUALIFIED_FACULTY': {
            ...
            if (candidates.length > 0) {
                const names = candidates.map((c) => `${c.faculty.lastName}, ${c.faculty.firstName}`).join('; ');
                suggestions.push({
        ...
        case 'NO_COMPATIBLE_ROOM': {
            ...
            if (compatibleRooms.length > 0) {
                const roomNames = compatibleRooms.map((r) => `${r.name} (${r.building.name})`).join('; ');
                suggestions.push({
...
//# sourceMappingURL=fix-suggestions.service.js.map