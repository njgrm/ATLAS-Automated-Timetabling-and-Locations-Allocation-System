...
export async function syncAdvisoryHgAssignments(schoolId) {
    ...
    for (const adviser of advisers) {
        const advisedSectionId = adviser.advisedSectionId;
    ...
    const activeAdvisedPairs = new Set(advisers.map((a) => `${a.id}:${a.advisedSectionId}`));
    ...
    for (const fs of systemHgAssignments) {
        const sectionsToRemove = fs.sectionIds.filter((sid) => !activeAdvisedPairs.has(`${fs.facultyId}:${sid}`));
        if (sectionsToRemove.length === 0)
            continue;
        const remainingSections = fs.sectionIds.filter((sid) => !sectionsToRemove.includes(sid));
        await prisma.$transaction(async (tx) => {
...
//# sourceMappingURL=hg-advisory.service.js.map