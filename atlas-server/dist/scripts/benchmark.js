...
// ─── Preflight checks ───
async function preflight(schoolId, schoolYearId) {
    const errors = [];
    ...
    if (subjectCount === 0)
        errors.push(`No active subjects found for school=${schoolId}.`);
    if (facultyCount === 0)
        errors.push(`No active faculty found for school=${schoolId}.`);
    if (roomCount === 0)
        errors.push(`No teaching rooms found for school=${schoolId}.`);
    if (sectionSourceMode === 'enrollpro') {
        errors.push(`SECTION_SOURCE_MODE=enrollpro requires a reachable EnrollPro API. ` +
...
async function main() {
    ...
    const artifactPath = path.join(artifactDir, `phase3-benchmark-${isoStamp}-school${schoolId}-year${schoolYearId}-runs${runCount}.json`);
    ...
    for (const run of report.runs) {
        ...
        console.log(`  Run #${run.runId}: ${run.status} | ${run.durationMs}ms | assigned=${run.assignedCount} unassigned=${run.unassignedCount} hardViolations=${run.hardViolationCount}${errorSuffix}`);
    }
    await prisma.$disconnect();
