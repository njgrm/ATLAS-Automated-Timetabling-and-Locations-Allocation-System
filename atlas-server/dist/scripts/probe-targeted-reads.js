...
function isRunPublished(summary) {
    return Boolean(summary && typeof summary === 'object' && summary.isPublished === true);
}
function entryIds(entries) {
    return entries.map((entry) => entry.entryId);
}
function assertEqualIds(label, actual, expected) {
...
async function main() {
    ...
    if (!facultyEntry)
        throw new Error('Published payload has no faculty-linked entry to probe.');
    if (!roomEntry)
        throw new Error('Published payload has no room-linked entry to probe.');
    if (!sectionEntry)
        throw new Error('Published payload has no section-linked entry to probe.');
    ...
    if (facultyId == null)
        throw new Error('Selected faculty probe entry has no faculty ID.');
    if (roomId == null)
        throw new Error('Selected room probe entry has no room ID.');
    if (sectionId == null)
        throw new Error('Selected section probe entry has no section ID.');
    ...
    assertEqualIds(`faculty ${facultyId}`, facultyPayload.entries, fullEntries.filter((entry) => entry.faculty.id === facultyId));
    assertEqualIds(`room ${roomId}`, roomPayload.entries, fullEntries.filter((entry) => entry.room.id === roomId));
    assertEqualIds(`section ${sectionId}`, sectionPayload.entries, fullEntries.filter((entry) => entry.section.id === sectionId));
...
//# sourceMappingURL=probe-targeted-reads.js.map