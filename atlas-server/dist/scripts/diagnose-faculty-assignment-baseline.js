...
import { computeTeachingLoadMinutes, detectDuplicateOwnershipTuples, } from '../services/faculty-assignment.service.js';
...
async function run() {
    ...
    const duplicateOwnership = detectDuplicateOwnershipTuples(assignments.map((assignment) => ({
    ...
    const focusedAssignments = new Map();
    ...
    if (report.focusedCases.length > 0) {
        ...
        for (const focused of report.focusedCases) {
            ...
            console.log(`- ${s.fullName} (#${s.facultyId}): sectionHours=${s.sectionHours}, gradeHours=${s.gradeHours}, delta=${s.deltaHours}`);
        }
    }
...
//# sourceMappingURL=diagnose-faculty-assignment-baseline.js.map