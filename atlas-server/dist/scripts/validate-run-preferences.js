...
async function main() {
    ...
    const facultyOutsideExplicit = new Map();
    for (const entry of entries) {
        ...
        if (entry.subjectId && !assignmentSet.has(`${entry.facultyId}:${entry.subjectId}`)) {
            if (!facultyOutsideExplicit.has(entry.facultyId)) {
                facultyOutsideExplicit.set(entry.facultyId, new Set());
            }
    ...
    const nonSpecialSectionRooms = new Map();
    for (const entry of entries) {
        if (!entry.sectionId || !entry.roomId)
            continue;
        ...
        if (isSpecialReason || isSpecialRoom)
            continue;
        if (!nonSpecialSectionRooms.has(entry.sectionId)) {
            nonSpecialSectionRooms.set(entry.sectionId, new Set());
        }
    ...
    const aquinoElpidio = facultyRows.filter((f) => normalize(f.lastName) === 'AQUINO' && normalize(f.firstName) === 'ELPIDIO');
    ...
    for (const member of aquinoElpidio) {
        ...
        const effective = Math.max(0, maxMinutes - ancillary);
        const outside = [...(facultyOutsideExplicit.get(member.id) ?? new Set())]
            .map((subjectId) => subjectById.get(subjectId)?.code ?? subjectById.get(subjectId)?.name ?? String(subjectId))
            .sort();
        console.log(`- id=${member.id} assigned=${assigned} effective=${effective} outsideExplicitSubjects=${outside.length} [${outside.join(', ')}]`);
    }
}
main()
    .catch((error) => {
...
//# sourceMappingURL=validate-run-preferences.js.map