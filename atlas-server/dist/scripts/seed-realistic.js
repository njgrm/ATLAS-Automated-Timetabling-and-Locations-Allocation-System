...
function buildGradeLevelRooms(gradeLevel, numRoomsPerFloor) {
    const rooms = [];
...
const REALISTIC_CAMPUS_BUILDINGS = [
...
function createTeacherExternalId(schoolId, teacherIndex) {
    return schoolId * 10000 + teacherIndex + 1;
}
function roomStableKey(name, floor) {
    return `${floor}:${name.trim().toLowerCase()}`;
}
function generateSectionsByGrade() {
    ...
    return gradeBlueprints.map((grade, index) => ({
        ...
        sections: grade.sections.map((section) => ({
            ...
            gradeLevelName: grade.gradeLevelName,
            displayOrder: grade.displayOrder,
            programType: (section.programCode ?? 'REGULAR'),
...
function generateTeachers(schoolId, sections) {
...
function generateTleCohorts(gradeLevels) {
...
async function collectExistingState(schoolId, schoolYearId) {
...
function printPreflightSummary(options, school, state) {
    const resetActions = [
...
async function resetWaveData(options) {
...
async function resetMapData(schoolId) {
    await prisma.$transaction([
...
async function upsertFacultyMirrors(schoolId, teachers) {
...
async function saveFacultySnapshot(schoolId, schoolYearId, teachers) {
...
async function saveSectionSnapshot(schoolId, schoolYearId, gradeLevels) {
...
function resolveEnrollProAuthToken(options) {
    ...
    return {
        token: jwt.sign({ userId: options.authUserId, role: options.authRole }, jwtSecret, { expiresIn: '15m' }),
        source: 'generated-jwt',
    };
}
function assertNonFixtureSource(domain, source) {
...
async function upsertCohorts(schoolId, schoolYearId, gradeLevels) {
...
async function seedCampusMap(schoolId, resetMap) {
    if (resetMap) {
                ...
                data: {
                    ...
                    rooms: {
                        create: building.rooms.map((room) => ({
                            ...
                            isTeachingSpace: building.isTeachingBuilding === false || NON_TEACHING_ROOM_TYPES.has(room.type)
                                ? false
        ...
        if (missingRooms.length > 0) {
            const created = await prisma.room.createMany({
                data: missingRooms.map((room) => ({
                    ...
                    capacity: room.capacity,
                    floorPosition: room.floorPosition,
                    isTeachingSpace: existing.isTeachingBuilding === false || NON_TEACHING_ROOM_TYPES.has(room.type)
                        ? false
                        : room.isTeachingSpace ?? true,
                })),
            });
