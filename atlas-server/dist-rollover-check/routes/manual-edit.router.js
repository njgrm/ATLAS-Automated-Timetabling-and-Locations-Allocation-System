...
router.post('/:schoolId/:schoolYearId/runs/:runId/manual-edits/preview', authenticate, async (req, res, next) => {
    try {
        ...
        const result = await manualEditService.previewManualEdit(scope.runId, scope.schoolId, scope.schoolYearId, proposal);
        res.json(result);
    }
    catch (e) {
        next(e);
...
router.post('/:schoolId/:schoolYearId/runs/:runId/manual-edits/commit', authenticate, async (req, res, next) => {
    try {
        const role = req.user?.role;
        if (!role || !PRIVILEGED_ROLES.has(role)) {
        ...
        const result = await manualEditService.commitManualEdit(scope.runId, scope.schoolId, scope.schoolYearId, actorId, proposal, expectedVersion, !!allowSoftOverride);
        res.json(result);
    }
    catch (e) {
        next(e);
...
router.post('/:schoolId/:schoolYearId/runs/:runId/manual-edits/batch/preview', authenticate, async (req, res, next) => {
    try {
        const role = req.user?.role;
        if (!role || !PRIVILEGED_ROLES.has(role)) {
        ...
        const result = await manualEditService.previewManualEditBatch(scope.runId, scope.schoolId, scope.schoolYearId, proposals);
        res.json(result);
    }
    catch (e) {
        next(e);
...
router.post('/:schoolId/:schoolYearId/runs/:runId/manual-edits/batch/commit', authenticate, async (req, res, next) => {
    try {
        const role = req.user?.role;
        if (!role || !PRIVILEGED_ROLES.has(role)) {
        ...
        const result = await manualEditService.commitManualEditBatch(scope.runId, scope.schoolId, scope.schoolYearId, actorId, proposals, expectedVersion, !!allowSoftOverride);
        res.json(result);
    }
    catch (e) {
        next(e);
...
router.post('/:schoolId/:schoolYearId/runs/:runId/manual-edits/revert', authenticate, async (req, res, next) => {
    try {
        const role = req.user?.role;
        if (!role || !PRIVILEGED_ROLES.has(role)) {
        ...
        const result = await manualEditService.revertLastEdit(scope.runId, scope.schoolId, scope.schoolYearId, actorId);
        res.json(result);
    }
    catch (e) {
        next(e);
...
router.get('/:schoolId/:schoolYearId/runs/:runId/manual-edits', authenticate, async (req, res, next) => {
    try {
        const role = req.user?.role;
        if (!role || !PRIVILEGED_ROLES.has(role)) {
        ...
        const edits = await manualEditService.listManualEdits(scope.runId, scope.schoolId, scope.schoolYearId);
