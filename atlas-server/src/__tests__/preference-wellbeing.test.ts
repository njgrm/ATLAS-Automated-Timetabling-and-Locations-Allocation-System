...
async function run() {
	...
	try {
		...
		const draftRes = await requestJson(
			...
			{ method: 'PUT', headers: authHeader, body: JSON.stringify(draftPayload) },
		);
		...
		const draftVersion = draftRes.json?.preference?.version as number;
		...
		const submitRes = await requestJson(
			...
			{ method: 'POST', headers: authHeader, body: JSON.stringify(submitPayload) },
		);
		...
		const submittedVersion = submitRes.json?.preference?.version as number;
		...
		const reDraftRes = await requestJson(
			baseUrl,
			`/preferences/${schoolId}/${syId}/faculty/${facultyId}/draft`,
			{ method: 'PUT', headers: authHeader, body: JSON.stringify(reDraftPayload) },
		);
