...
async function run() {
	...
	const programType = (sectionMirror.programType ?? 'REGULAR') as ProgramType;

	const runIds: number[] = [];
	const cleanup = {
		authAccountId: null as number | null,
		facultyIds: [] as number[],
		facultySubjectId: null as number | null,
	...
	try {
		...
		if (!token) throw new Error('Faculty login did not return a token.');
