...
function run() {
	...
	assert(
		normalizedSteEntries.length > 0 && normalizedSteEntries.every((entry) => normalizedSteSlotKeys.has(`${entry.startTime}-${entry.endTime}`)),
		'normalized grade-level IDs still resolve to the correct program shape slots',
	);
