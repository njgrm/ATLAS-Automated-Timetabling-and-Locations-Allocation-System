...
async function run() {
	...
	try {
		...
		const actualRequestVersion = firstSubmit.json?.entries?.find((entry: { entryId: string; version?: number | null }) => entry.entryId === entryId)?.version;
		...
		const staleRequestVersionSubmit = await requestJson(baseUrl, `/room-preferences/${schoolId}/${schoolYearId}/runs/${run.id}/faculty/${faculty.facultyId}/entries/${entryId}/submit`, {
			...
			body: JSON.stringify({
				...
				expectedRunVersion: run.version,
				requestVersion: (actualRequestVersion as number) + 999,
			}),
		});
