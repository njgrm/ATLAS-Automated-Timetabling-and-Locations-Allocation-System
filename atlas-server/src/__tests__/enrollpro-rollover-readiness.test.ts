...
async function fetchEnrollProJson(path: string): Promise<unknown> {
...
async function fetchEnrollProStatus(path: string): Promise<number> {
	const response = await fetch(`${enrollProBaseUrl()}${path}`, { signal: AbortSignal.timeout(10000) });
...
function assertSectionContract(row: unknown, label: string) {
...
function assertFacultyContract(row: unknown, label: string) {
...
function handleFakeEnrollProRolloverRequest(req: IncomingMessage, res: ServerResponse) {
...
async function startFakeEnrollProRolloverServer(): Promise<{ server: Server; baseUrl: string }> {
...
async function run() {
	...
	try {
		...
		await expectErrorCode(
			() => triggerGenerationRun(schoolId, activeSchoolYearId, actorId),
			'ACTIVE_YEAR_DRIFT',
			'Generation against the old active year is blocked after simulated EnrollPro rollover',
		);
	} finally {
