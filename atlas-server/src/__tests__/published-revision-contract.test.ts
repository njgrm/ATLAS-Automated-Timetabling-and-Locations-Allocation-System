...
async function requestJson(baseUrl: string, path: string, options?: RequestInit): Promise<JsonResponse> {
...
async function expectServiceCode(label: string, expectedCode: string, action: () => Promise<unknown>) {
	try {
		await action();
		assert(false, `${label} throws ${expectedCode}`);
	} catch (error) {
		assertEqual((error as { code?: string }).code, expectedCode, `${label} throws ${expectedCode}`);
	}
}
...
async function run() {
	...
	try {
		...
		assertEqual((preservedRun.summary as { isPublished?: boolean }).isPublished, true, 'Source run remains published');
		...
		assertEqual((audit.metadata as { publishedTruthPreserved?: boolean }).publishedTruthPreserved, true, 'Audit metadata records published truth preservation');
