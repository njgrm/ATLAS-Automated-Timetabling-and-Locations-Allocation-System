...
async function run() {
	...
	try {
		...
		const submit = await requestJson(
			baseUrl,
			`/room-preferences/${schoolId}/${schoolYearId}/runs/${run.id}/faculty/${facultyAccount.facultyId}/entries/${latestState.json?.entries?.[0]?.entryId}/submit`,
			{
				method: 'POST',
				headers: {
		...
		const requestId = summary.json?.requests?.[0]?.id as number | undefined;
