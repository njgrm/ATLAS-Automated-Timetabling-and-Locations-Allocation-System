import { computeDemand, constructBaseline, type ConstructorInput, type SubjectInput } from '../services/schedule-constructor.js';
...
function run() {
  ...
  const hasQuarterKey = (modularEntry?.metadata?.modularAssignments ?? []).some((assignment) => 'quarter' in (assignment as Record<string, unknown>));
