...
async function readUntil(reader: ReadableStreamDefaultReader<Uint8Array>, matcher: (text: string) => boolean, timeoutMs: number): Promise<string | null> {
...
async function run() {
	...
	try {
		...
		const token = officerLogin.json?.token as string | undefined;
		...
		await requestJson(
			baseUrl,
			`/room-preferences/${facultyAccount.schoolId}/1/runs/${run.id}/faculty/${facultyAccount.facultyId}/sync`,
			{
				method: 'POST',
				headers: {
