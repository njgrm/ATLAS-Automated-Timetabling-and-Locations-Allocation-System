...
function run() {
...

	{
		...
		assert(
			result.unassignedItems[0]?.homeRoomFallbackCause === 'FACULTY_DAILY_LIMIT_EXCEEDED',
			'Faculty overload maps to explicit FACULTY_DAILY_LIMIT_EXCEEDED fallback cause',
		);
	}
