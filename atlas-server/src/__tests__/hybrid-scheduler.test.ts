...
import {
	...
	type SeedProfileId,
	type RepairImpact,
} from '../services/hybrid-scheduler.js';
import type { ConstructorResult } from '../services/schedule-constructor.js';
...
function makeGrade(
	displayOrder: number,
	sections: Array<{ id: number; name: string; enrolledCount: number; adviserId?: number | null; adviserName?: string | null }>,
): SectionsByGrade {
	return {
...
function assertLessThan(actual: number, threshold: number, label: string) {
	if (actual < threshold) {
		passCount++;
		console.log(`  ✓ ${label} (${actual} < ${threshold})`);
	} else {
...
section('H-ALG-3-02 repairHardConflicts detects faculty-time conflict');
{
	...
	assert(
		!(e1.facultyId === e2.facultyId && e1.day === e2.day && e1.startTime === e2.startTime),
		'Faculty-time conflict resolved: e1 and e2 no longer overlap',
	);
}
...
section('H-ALG-3-03 repairHardConflicts detects room-time conflict');
{
	...
	const e2 = result.find((e) => e.entryId === 'e2')!;
	assert(
		!(e1.roomId === e2.roomId && e1.day === e2.day && e1.startTime === e2.startTime),
		'Room-time conflict resolved: e1 and e2 no longer share room+slot',
	);
}
...
section('H-ALG-3-05 repairHardConflicts no mutation of original array');
{
	...
	assert(
		entries[0].day === originalSnapshot[0].day && entries[0].startTime === originalSnapshot[0].startTime,
		'Original entries array not mutated (entry 0)',
	);
}
...
section('H-ALG-1-02 runHybridScheduler seedQuality profiles are deterministic');
{
	const input: ConstructorInput = {
		...
		subjects: [
			{ id: 1, code: 'MATH', minMinutesPerWeek: 150, preferredRoomType: 'CLASSROOM' as const, sessionPattern: 'MWF' as const, gradeLevels: [7] },
			{ id: 2, code: 'SCI', minMinutesPerWeek: 100, preferredRoomType: 'LABORATORY' as const, sessionPattern: 'TTH' as const, gradeLevels: [7] },
		],
		faculty: [],
		facultySubjects: [],
		rooms: [],
...
section('H-ALG-1-03 runHybridScheduler selects profile with fewest unassigned');
{
	...
	const input: ConstructorInput = {
		...
		subjects: [
			{ id: 1, code: 'ENG', minMinutesPerWeek: 200, preferredRoomType: 'CLASSROOM' as const, sessionPattern: 'ANY' as const, gradeLevels: [7, 8] },
		],
		faculty: [],
		facultySubjects: [],
		rooms: [],
