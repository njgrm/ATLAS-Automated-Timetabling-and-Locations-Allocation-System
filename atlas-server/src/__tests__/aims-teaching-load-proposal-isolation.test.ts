...
async function snapshotTlCounts(schoolId: number, schoolYearId: number) {
...
async function snapshotPublishedSchedule(schoolId: number) {
	...
	return {
		...
		entryCount: publishedRuns[0]
			? Array.isArray((publishedRuns[0].draftEntries as unknown[]))
				? ((publishedRuns[0].draftEntries as unknown[])).length
				: 0
			: 0,
	};
}
...
async function run() {
	...
	let activeSchoolYearId: number | null = null;
	try {
