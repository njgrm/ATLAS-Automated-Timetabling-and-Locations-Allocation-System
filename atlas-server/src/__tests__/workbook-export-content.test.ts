...
async function run() {
	...
	const runId = run.id;
	const entries = (run.draftEntries ?? []) as unknown as Array<{
		sectionId: number;
		subjectId: number;
	...
	summarySheet.eachRow((row) => {
		row.eachCell({ includeEmpty: false }, (cell) => {
			const v = cell.value;
			if (typeof v === 'string') allCellValues.push(v);
		});
	});
	...
	const lastNames = faculty.map((f) => f.lastName).filter(Boolean) as string[];
	...
	await classProgWb.xlsx.load(classProgBuffer as any);
	...
	if (!classProgSheet) { process.exit(1); }

	// Check BLDG./RM. row
	const cpAllCellValues: string[] = [];
	...
	classProgSheet.eachRow((row, rowNumber) => {
		...
		row.eachCell({ includeEmpty: false }, (cell) => {
			const v = cell.value;
			if (typeof v === 'string') cpAllCellValues.push(v);
		});
	});
	...
	if (entriesHaveRooms) {
		...
		classProgSheet.eachRow((row) => {
			if (bldgRowFound) return;
			const firstCell = row.getCell(1).value;
			if (typeof firstCell === 'string' && firstCell === 'BLDG./RM.') {
				bldgRowFound = true;
				const bldgCellValues: string[] = [];
				row.eachCell({ includeEmpty: false }, (cell) => {
					const v = cell.value;
					if (typeof v === 'string' && v !== 'BLDG./RM.') bldgCellValues.push(v);
				});
