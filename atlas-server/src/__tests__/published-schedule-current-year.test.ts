/**
 * Published Schedule Current-Year Guard Test
 *
 * Proves that the default published schedule endpoint only returns
 * the current active school year's published schedule, not historical runs.
 *
 * Test flow:
 *   1. Resolve active school year from EnrollPro mirror.
 *   2. Verify default endpoint returns active-year published run.
 *   3. Verify explicit historical year endpoint still works.
 *   4. Verify 404 when no active-year published run exists.
 */
