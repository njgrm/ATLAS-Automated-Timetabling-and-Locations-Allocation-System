...
// section-adapter.ts.  We set env vars first, then dynamically import the
// router so the adapter resolves to stub mode.
const JWT_SECRET = 'section-route-authority-c01-test-secret';
const SYSTEM_TOKEN = 'test-system-token-for-section-route-authority';

process.env.JWT_SECRET = JWT_SECRET;
process.env.ATLAS_SYSTEM_TOKEN = SYSTEM_TOKEN;
// Stub mode: the section adapter returns deterministic data without contacting
// any upstream EnrollPro host.  This makes F2-SYNC a genuine 200 success.
...
// ==== E6: System token paths (restored additive row) ========================

test('E6a: system token with explicit schoolId on POST /sync reaches the service (success)', async () => {
	const recording = makeRecordingClient();
	const { server, baseUrl } = await startServer(recording);
	try {
		const result = await callRoute(baseUrl, 'POST', '/api/v1/sections/sync', systemTokenValue(), { schoolId: 1, schoolYearId: 1 });
		assert.equal(result.status, 200, `E6a -> expected 200, got ${result.status}/${result.json.code ?? result.json.message}`);
		assert.ok(result.json.count !== undefined, 'E6a returns sync count');
		assert.equal(result.json.synced, true, 'E6a synced is true');
	} finally { server.close(); }
});

test('E6b: system token with no schoolId on POST /sync rejected with zero dispatch', async () => {
	const recording = makeRecordingClient();
	const { server, baseUrl } = await startServer(recording);
	try {
		const result = await callRoute(baseUrl, 'POST', '/api/v1/sections/sync', systemTokenValue(), {});
		assert.equal(result.status, 400, `E6b -> ${result.status}/${result.json.code}`);
		assert.equal(result.json.code, 'INVALID_PARAM');
		assert.equal(recording.ops.length, 0, 'E6b dispatches zero DB operations');
		assert.equal(recording.writes.length, 0, 'E6b performs zero writes');
	} finally { server.close(); }
});

test('E6c: system token on GET /home-rooms rejected (actor-only gate, no schoolId)', async () => {
	const recording = makeRecordingClient();
	const { server, baseUrl } = await startServer(recording);
	try {
		const result = await callRoute(baseUrl, 'GET', '/api/v1/sections/home-rooms/1?schoolId=1', systemTokenValue(), undefined);
		assert.equal(result.status, 401, `E6c -> ${result.status}/${result.json.code}`);
		assert.equal(result.json.code, 'INVALID_TOKEN');
		assert.equal(recording.ops.length, 0, 'E6c dispatches zero DB operations');
		assert.equal(recording.writes.length, 0, 'E6c performs zero writes');
	} finally { server.close(); }
});

test('E6d: system token on PUT /home-rooms rejected (actor-only gate, no schoolId)', async () => {
	const recording = makeRecordingClient();
	const { server, baseUrl } = await startServer(recording);
	try {
		const result = await callRoute(baseUrl, 'PUT', '/api/v1/sections/home-rooms/1', systemTokenValue(), { schoolId: 1, assignments: [{ sectionId: 1, homeRoomId: null }] });
		assert.equal(result.status, 401, `E6d -> ${result.status}/${result.json.code}`);
		assert.equal(result.json.code, 'INVALID_TOKEN');
		assert.equal(recording.ops.length, 0, 'E6d dispatches zero DB operations');
		assert.equal(recording.writes.length, 0, 'E6d performs zero writes');
	} finally { server.close(); }
});

// ==== E7: Missing / invalid JWT on all three routes =========================

test('E7a: no token on GET /home-rooms returns 401 NO_TOKEN with zero dispatch', async () => {
	const recording = makeRecordingClient();
	const { server, baseUrl } = await startServer(recording);
	try {
		const result = await callRoute(baseUrl, 'GET', '/api/v1/sections/home-rooms/1?schoolId=1', null, undefined);
		assert.equal(result.status, 401, `E7a -> ${result.status}/${result.json.code}`);
		assert.equal(result.json.code, 'NO_TOKEN');
		assert.equal(recording.ops.length, 0, 'E7a dispatches zero DB operations');
	} finally { server.close(); }
});

test('E7b: no token on PUT /home-rooms returns 401 NO_TOKEN with zero dispatch', async () => {
	const recording = makeRecordingClient();
	const { server, baseUrl } = await startServer(recording);
	try {
		const result = await callRoute(baseUrl, 'PUT', '/api/v1/sections/home-rooms/1', null, { schoolId: 1, assignments: [{ sectionId: 1, homeRoomId: null }] });
		assert.equal(result.status, 401, `E7b -> ${result.status}/${result.json.code}`);
		assert.equal(result.json.code, 'NO_TOKEN');
		assert.equal(recording.ops.length, 0, 'E7b dispatches zero DB operations');
	} finally { server.close(); }
});

test('E7c: no token on POST /sync returns 401 NO_TOKEN with zero dispatch', async () => {
	const recording = makeRecordingClient();
	const { server, baseUrl } = await startServer(recording);
	try {
		const result = await callRoute(baseUrl, 'POST', '/api/v1/sections/sync', null, { schoolId: 1 });
		assert.equal(result.status, 401, `E7c -> ${result.status}/${result.json.code}`);
		assert.equal(result.json.code, 'NO_TOKEN');
		assert.equal(recording.ops.length, 0, 'E7c dispatches zero DB operations');
	} finally { server.close(); }
});

test('E7d: invalid JWT on GET /home-rooms returns 401 INVALID_TOKEN with zero dispatch', async () => {
	const recording = makeRecordingClient();
	const { server, baseUrl } = await startServer(recording);
	try {
		const result = await callRoute(baseUrl, 'GET', '/api/v1/sections/home-rooms/1?schoolId=1', signInvalidToken(), undefined);
		assert.equal(result.status, 401, `E7d -> ${result.status}/${result.json.code}`);
		assert.equal(result.json.code, 'INVALID_TOKEN');
		assert.equal(recording.ops.length, 0, 'E7d dispatches zero DB operations');
	} finally { server.close(); }
});

test('E7e: invalid JWT on PUT /home-rooms returns 401 INVALID_TOKEN with zero dispatch', async () => {
	const recording = makeRecordingClient();
	const { server, baseUrl } = await startServer(recording);
	try {
		const result = await callRoute(baseUrl, 'PUT', '/api/v1/sections/home-rooms/1', signInvalidToken(), { schoolId: 1, assignments: [{ sectionId: 1, homeRoomId: null }] });
		assert.equal(result.status, 401, `E7e -> ${result.status}/${result.json.code}`);
		assert.equal(result.json.code, 'INVALID_TOKEN');
		assert.equal(recording.ops.length, 0, 'E7e dispatches zero DB operations');
	} finally { server.close(); }
});

test('E7f: invalid JWT on POST /sync returns 401 INVALID_TOKEN with zero dispatch', async () => {
	const recording = makeRecordingClient();
	const { server, baseUrl } = await startServer(recording);
	try {
		const result = await callRoute(baseUrl, 'POST', '/api/v1/sections/sync', signInvalidToken(), { schoolId: 1 });
		assert.equal(result.status, 401, `E7f -> ${result.status}/${result.json.code}`);
		assert.equal(result.json.code, 'INVALID_TOKEN');
		assert.equal(recording.ops.length, 0, 'E7f dispatches zero DB operations');
	} finally { server.close(); }
});
...
test('F2-PUT: same-school PUT /home-rooms/:schoolYearId reaches the service', async () => {
	...
	const { server, baseUrl } = await startServer(recording);
	try {
		const result = await callRoute(baseUrl, 'PUT', '/api/v1/sections/home-rooms/1', signOfficer(1), { schoolId: 1, assignments: [{ sectionId: 999999, homeRoomId: null }] });
...
test('F2-SYNC: same-school POST /sync is a genuine success (stub adapter, HTTP 200)', async () => {
	const recording = makeRecordingClient();
	const { server, baseUrl } = await startServer(recording);
	try {
		const result = await callRoute(baseUrl, 'POST', '/api/v1/sections/sync', signOfficer(1), { schoolId: 1, schoolYearId: 1 });
		assert.equal(result.status, 200, `POST /sync same-school returns 200 (got ${result.status}/${result.json.code ?? result.json.message})`);
		assert.equal(typeof result.json.synced, 'boolean', 'F2-SYNC returns synced boolean');
		assert.equal(typeof result.json.count, 'number', 'F2-SYNC returns count');
		assert.equal(typeof result.json.removed, 'number', 'F2-SYNC returns removed');
		assert.ok(recording.ops.length > 0, 'F2-SYNC reached the service (ops: ' + recording.ops.join(', ') + ')');
	} finally { server.close(); }
});
...
test('F2-GET: same-school GET /home-rooms/:schoolYearId reaches the service and returns valid shape', async () => {
	...
	const { server, baseUrl } = await startServer(recording);
	try {
		const result = await callRoute(baseUrl, 'GET', '/api/v1/sections/home-rooms/1?schoolId=1', signOfficer(1), undefined);
