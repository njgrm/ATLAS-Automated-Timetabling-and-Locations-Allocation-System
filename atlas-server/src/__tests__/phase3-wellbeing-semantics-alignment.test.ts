...
const PERIOD_SLOTS: Array<{ startTime: string; endTime: string }> = [
...
function buildUnavailableSlotsForConsecutivePressure(): PreferenceSlotInput[] {
	const blocked: PreferenceSlotInput[] = [];
...
function buildInput(enforceConsecutiveBreakAsHard: boolean): ConstructorInput {
	const lockedEntries = PERIOD_SLOTS.flatMap((slot, index) => {
		...
		return [
			...
			...(['TUESDAY', 'WEDNESDAY', 'THURSDAY', 'FRIDAY'] as const).map((day) => ({
				sectionId: 7701,
				subjectId: 9998,
				facultyId: 8899,
				roomId: 6601,
