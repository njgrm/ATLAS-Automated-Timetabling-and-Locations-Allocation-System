...
function waitForSocketEvent(ws: WebSocket, matcher: (payload: any) => boolean, timeoutMs = 4000): Promise<any> {
...
async function run() {
	...
	let officerWs: WebSocket | null = null;
	let facultyWs: WebSocket | null = null;
	let timeoutWs: WebSocket | null = null;
	...
	try {
		...
		const officerToken = officerLogin.json?.token as string | undefined;
		...
		const facultyToken = facultyLogin.json?.token as string | undefined;
		...
		if (officerWs && officerWs.readyState === WebSocket.OPEN) officerWs.close();
		if (facultyWs && facultyWs.readyState === WebSocket.OPEN) facultyWs.close();
		if (timeoutWs && timeoutWs.readyState === WebSocket.OPEN) timeoutWs.close();
