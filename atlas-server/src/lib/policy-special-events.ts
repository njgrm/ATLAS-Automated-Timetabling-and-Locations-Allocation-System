...
export const VALID_EVENT_TYPES = ['FLAG_OR_HGP', 'HEALTH_BREAK', 'LUNCH_BREAK', 'CUSTOM'] as const;
export type SpecialEventType = (typeof VALID_EVENT_TYPES)[number];

export const VALID_GRADE_GROUPS = ['7-8', '9-10'] as const;
 ...
 */
export function resolveGradeGroup(gradeLevel: number): GradeGroup | null {
	if (gradeLevel >= 7 && gradeLevel <= 8) return '7-8';
	if (gradeLevel >= 9 && gradeLevel <= 10) return '9-10';
	return null;
}
...
export function getEffectiveEvents(
	specialEvents: SpecialEventRowLike[],
	gradeLevel: number,
	programType?: string | null,
): EffectiveSpecialEvent[] {
	...
	const result: EffectiveSpecialEvent[] = [];
	...
	for (const eventType of VALID_EVENT_TYPES) {
		...
		const tier1 = rows.find(
			(r) => r.gradeGroup != null && r.gradeGroup === resolvedGradeGroup
		...
		const tier4 = rows.find(
			(r) => r.gradeGroup == null && r.programType == null,
		);
		if (tier4) {
...
function toEffective(row: SpecialEventRowLike): EffectiveSpecialEvent {
	return {
