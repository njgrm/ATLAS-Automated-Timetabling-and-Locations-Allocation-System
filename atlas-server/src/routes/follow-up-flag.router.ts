...
router.get(
	'/:schoolId/:schoolYearId/runs/:runId/flags',
	authenticate,
	async (req: Request, res: Response, next: NextFunction) => {
		try {
			const schoolId = positiveInt(req.params.schoolId, 'schoolId');
			if (typeof schoolId === 'string') { res.status(400).json({ code: 'BAD_PARAM', message: schoolId }); return; }
			const schoolYearId = positiveInt(req.params.schoolYearId, 'schoolYearId');
			if (typeof schoolYearId === 'string') { res.status(400).json({ code: 'BAD_PARAM', message: schoolYearId }); return; }
			const runId = positiveInt(req.params.runId, 'runId');
			if (typeof runId === 'string') { res.status(400).json({ code: 'BAD_PARAM', message: runId }); return; }
...
router.put(
	'/:schoolId/:schoolYearId/runs/:runId/flags/:entryId',
	authenticate,
	async (req: Request, res: Response, next: NextFunction) => {
		try {
			const schoolId = positiveInt(req.params.schoolId, 'schoolId');
			...
			if (!entryId) { res.status(400).json({ code: 'BAD_PARAM', message: 'Invalid entryId.' }); return; }
