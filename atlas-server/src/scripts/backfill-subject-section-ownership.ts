...
type ConflictEntry = {
  ...
  owners: Array<{
    ...
    facultyName: string;
    assignedAt: string;
    action: 'inserted' | 'skipped-duplicate';
  }>;
};
...
const DEFAULT_OUTPUT_PATH = path.resolve(
  __dirname,
  '../../../qa-artifacts/subject-section-ownership-backfill-ledger.json',
);

function parseArgs(argv: string[]): CliOptions {
  let schoolIds: number[] = [1];
...
async function backfillSchool(schoolId: number, apply: boolean): Promise<SchoolResult> {
  ...
  const tripleOwnerMap = new Map<string, InsertRow[]>();
  ...
  const conflictedFsIds = new Set<number>();
  ...
  for (const [, owners] of tripleOwnerMap) {
    ...
    const facultyNameCache = new Map<number, string>();
    for (const row of rows) {
  ...
  const safeRows = insertRows.filter(
    (r) =>
      !conflictedFsIds.has(r.facultySubjectId) ||
  ...
  if (apply && safeRows.length > 0) {
    ...
    for (let i = 0; i < safeRows.length; i += BATCH) {
      ...
      const result = await (prisma.subjectSectionOwnership as any).createMany({
        data: batch.map((r) => ({
          schoolId: r.schoolId,
...
async function run(): Promise<void> {
  ...
  for (const schoolId of targetSchoolIds) {
    ...
    console.log(
      `  Ownership rows ${options.apply ? 'inserted' : 'would insert'}: ${result.rowsInserted}`,
    );
  ...
  let postCount = 0;
  if (options.apply) {
    const agg = await (prisma.subjectSectionOwnership as any).count({
  ...
  console.log(`${'─'.repeat(60)}`);
  console.log(
    `  subject_section_ownerships ${options.apply ? 'count' : 'would-be count'}: ${postCount}`,
  );
  console.log(
    `  Ownership conflicts: ${totalConflicts}${totalConflicts === 0 ? ' ✓' : ' ✗  (see ledger for details)'}`,
  );
