...
type FacultySubjectRow = {
  ...
  subject: { code: string; name: string; minMinutesPerWeek: number };
};
...
type RepairOpUpdate = {
  ...
  oldGradeLevels: number[];
  newGradeLevels: number[];
  reasons: string[];
};
...
type RepairOp = RepairOpUpdate | RepairOpDelete;

type ConflictResolutionEntry = {
  subjectId: number;
  sectionId: number;
  winner: { facultyId: number; facultyName: string; assignedAt: string };
  losers: Array<{ facultyId: number; facultyName: string; assignedAt: string }>;
  policy: string;
};
...
const DEFAULT_OUTPUT_PATH = path.resolve(
  __dirname,
  '../../../qa-artifacts/faculty-assignment-repair-ledger.json',
);
...
function sortedInts(values: number[]): number[] {
...
async function run(): Promise<void> {
  ...
  const rows: FacultySubjectRow[] = await prisma.facultySubject.findMany({
  ...
  const sectionDisplayOrderMap = new Map<number, number>(
    allMirrors.map((m) => [m.externalId, m.displayOrder]),
  );
  ...
  const sectionsByGradeLatestSY = new Map<number, number[]>();
  ...
  const rowByKey = new Map<string, FacultySubjectRow>(
    rows.map((row) => [`${row.facultyId}:${row.subjectId}`, row]),
  );

  const removalsByRowId = new Map<number, Set<number>>();
  const conflictResolutions: ConflictResolutionEntry[] = [];
  ...
  for (const tuple of duplicateTuples) {
    ...
    for (const loser of losers) {
      const removals = removalsByRowId.get(loser.rowId) ?? new Set<number>();
  ...
  const repairOps: RepairOp[] = [];

  for (const row of rows) {
    const facultyName = formatFullName(row.faculty.firstName, row.faculty.lastName);
    const removalSet = removalsByRowId.get(row.id) ?? new Set<number>();
    const reasons: string[] = [];

    // Apply conflict removals to compute effective sectionIds
    let effectiveSectionIds = sortedInts(
      row.sectionIds.filter((sid) => !removalSet.has(sid)),
    );

    if (removalSet.size > 0) {
      reasons.push(
        `conflict-removal: stripped ${removalSet.size} sectionId(s) [${[...removalSet].sort((a, b) => a - b).join(',')}] — lost ownership to earlier-assigned faculty`,
      );
    }
    ...
    if (effectiveSectionIds.length === 0 && row.gradeLevels.length > 0) {
      if (latestSchoolYearId !== null) {
        ...
        if (expanded.length > 0) {
          ...
          reasons.push(
            `legacy-expand: populated sectionIds from gradeLevels [${row.gradeLevels.join(',')}] via schoolYear ${latestSchoolYearId} (${expanded.length} sections)`,
          );
        } else {
          repairOps.push({
            type: 'DELETE',
    ...
    const derivedGradeLevels = deriveGradeLevelsFromSectionIds(
      effectiveSectionIds,
      sectionDisplayOrderMap,
    );

    const gradeLevelsDiffer = !arraysEqual(
      sortedInts(row.gradeLevels),
      sortedInts(derivedGradeLevels),
    );
    ...
    if (gradeLevelsDiffer && reasons.every((r) => !r.startsWith('legacy-expand'))) {
      reasons.push(
        `grade-normalise: stored [${sortedInts(row.gradeLevels).join(',')}] → derived [${derivedGradeLevels.join(',')}]`,
      );
    }
  ...
  const updateOps = repairOps.filter((op): op is RepairOpUpdate => op.type === 'UPDATE');
  const deleteOps = repairOps.filter((op): op is RepairOpDelete => op.type === 'DELETE');
  ...
  const validationRows = options.apply
    ? postRows
    : postRows.map((row) => {
        const op = repairOps.find(
          (o) => o.facultyId === row.facultyId && o.subjectId === row.subjectId,
        );
        if (!op) return row;
  ...
  const postDuplicates = detectDuplicateOwnershipTuples(
  ...
  const postByFaculty = new Map<number, typeof validationRows>();
  ...
  for (const assignments of postByFaculty.values()) {
    ...
    if (sectionMins !== gradeMins) postMismatchCount++;
  }
  ...
  console.log(`${'─'.repeat(60)}`);
  console.log(
    `  Remaining duplicate ownership tuples : ${postValidation.remainingDuplicateOwnershipTuples}${postValidation.remainingDuplicateOwnershipTuples === 0 ? ' ✓' : ' ✗'}`,
  );
  console.log(
    `  Faculty with section≠grade load mismatch: ${postValidation.remainingMismatchedLoadFaculty}`,
  );
  ...
  if (conflictResolutions.length > 0 && !options.apply) {
    ...
    for (const entry of conflictResolutions.slice(0, 10)) {
      console.log(
        `    subject=${entry.subjectId}, section=${entry.sectionId} → winner=#${entry.winner.facultyId} ${entry.winner.facultyName}`,
      );
    }
