...
async function main() {
  ...
  const entries = Array.isArray(latest.draftEntries) ? (latest.draftEntries as any[]) : [];
  ...
  for (const member of faculty) {
    ...
    console.log(
      `${member.lastName}, ${member.firstName} (id=${member.id}) assigned=${assignedMinutes} max=${maxMinutes} effective=${effectiveMinutes}`,
    );
  }
}
