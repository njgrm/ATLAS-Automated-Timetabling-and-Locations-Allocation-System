...
export interface BenchmarkStats {
	...
	assignedCount: { min: number; max: number; mean: number };
	unassignedCount: { min: number; max: number; mean: number };
	hardViolationCount: { min: number; max: number; mean: number };
	policyBlockedCount: { min: number; max: number; mean: number };
}
...
function minMaxMean(values: number[]): { min: number; max: number; mean: number } {
...
export async function runBenchmark(
	...
	for (let i = 0; i < runCount; i++) {
		...
		const summary = (completed.summary as unknown ?? {}) as RunSummary;
		runs.push({
			runId: completed.id,
