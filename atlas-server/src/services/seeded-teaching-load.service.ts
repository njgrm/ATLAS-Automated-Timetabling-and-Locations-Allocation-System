...
type SectionSubjectPair = {
	key: string;
	section: ExternalSection & { displayOrder: number };
	subject: SeedableSubject;
	candidateIds: number[];
	minutes: number;
};
...
export interface SeededTeachingLoadDiagnostics {
	...
	unassignedPairs: Array<{ sectionId: number; sectionName: string; subjectCode: string; subjectName: string }>;
	facultyWithoutAssignments: Array<{ facultyId: number; facultyName: string; department: string | null }>;
}
...
const SUBJECT_CODE_MATCHERS: Record<string, string[]> = {
...
function subjectMatchesDepartment(normalizedDepartment: string, subject: Pick<SeedableSubject, 'code' | 'name'>): boolean {
	const code = subject.code.trim().toLowerCase();
	const name = subject.name.trim().toLowerCase();
...
function supportsSecondaryCoverage(department: string | null, subject: Pick<SeedableSubject, 'code' | 'name'>): boolean {
...
function canTeachOutsideDepartment(subject: Pick<SeedableSubject, 'code' | 'name'>): boolean {
...
function subjectSort(left: SeedableSubject, right: SeedableSubject): number {
...
function sectionSort(left: ExternalSection & { displayOrder: number }, right: ExternalSection & { displayOrder: number }): number {
	return left.displayOrder - right.displayOrder || left.name.localeCompare(right.name) || left.id - right.id;
}

function pairSort(left: SectionSubjectPair, right: SectionSubjectPair): number {
...
function buildLoadStats(loadHours: number[]): SeededTeachingLoadStats {
...
async function loadGradeLevels(input: Pick<SeedTeachingLoadBaselineInput, 'schoolId' | 'schoolYearId' | 'authToken' | 'gradeLevels'>) {
...
function buildPairDefinitions(
	...
	const pairDefinitions: SectionSubjectPair[] = [];
...
function getFacultyCapacityMinutes(member: SeedableFaculty, maxWeeklyHoursCap: number): number {
	return Math.min(member.maxHoursPerWeek, maxWeeklyHoursCap) * 60;
}

function printSeedDiagnostics(diagnostics: SeededTeachingLoadDiagnostics) {
...
export async function collectSeededTeachingLoadDiagnostics(input: Pick<SeedTeachingLoadBaselineInput, 'schoolId' | 'schoolYearId' | 'authToken' | 'gradeLevels'>): Promise<SeededTeachingLoadDiagnostics> {
	...
	const sectionCounts = new Map<number, Set<number>>();
	...
	for (const assignment of assignments) {
		...
		const facultySections = sectionCounts.get(assignment.facultyId) ?? new Set<number>();
	...
	const facultyRows: FacultyLoadRow[] = faculty.map((member) => {
		...
		return {
			...
			assignedSections: (sectionCounts.get(member.id) ?? new Set<number>()).size,
...
export async function seedTeachingLoadBaseline(input: SeedTeachingLoadBaselineInput): Promise<SeedTeachingLoadBaselineResult> {
	...
	const sectionsByFacultySubject = new Map<string, { facultyId: number; subjectId: number; sectionIds: Set<number>; gradeLevels: Set<number> }>();

	function canFit(facultyId: number, minutes: number) {
	...
	function assignPair(facultyId: number, pair: SectionSubjectPair) {
