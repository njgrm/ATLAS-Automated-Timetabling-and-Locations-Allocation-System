...
function findSuggestedPlacements(
	...
	projectedEntries: ScheduledEntry[],
	activeTimeSlots: Array<{ startTime: string; endTime: string }>,
	schoolId: number,
	schoolYearId: number,
	runId: number,
	...
	const suggestedPlacements: ManualEditProposal[] = top3.map((s) => ({
...
async function validateExpectedFacultyVersions(
	...
	options: { targetFacultyIds?: Set<number> } = {},
	client: Prisma.TransactionClient | typeof prisma = prisma,
): Promise<Map<number, number>> {
	const uniqueFacultyIds = [...new Set(facultyIds)];
	const targetFacultyIds = options.targetFacultyIds ?? new Set<number>(uniqueFacultyIds);
...
async function prepareRepair(
	...
	const targetFacultyIds = new Set(changes.map((change) => change.toFacultyId).filter((id): id is number => id != null));
	...
	const unassignedReadiness = normalizedChanges
		...
		.map<TeachingLoadUnassignedReadiness>((change) => {
			...
			const searchRes = findSuggestedPlacements(
				change,
				projectedRefData,
				newEntries,
				activeTimeSlots,
