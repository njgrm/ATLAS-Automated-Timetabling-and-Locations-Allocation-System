 ...
 */
export async function syncAdvisoryHgAssignments(
	schoolId: number,
	schoolYearId: number,
): Promise<HgSyncSummary> {
	...
	const advisers = allAdvisers.filter((a) =>
		currentYearSectionIdSet.has(a.advisedSectionId as number),
	);
