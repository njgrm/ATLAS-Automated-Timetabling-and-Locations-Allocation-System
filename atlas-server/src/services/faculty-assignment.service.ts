...
type RotationTermBucketAccumulator = {
  termRank: number | null;
  termLabel: string | null;
  termGroupId: string | null;
  termCount: number | null;
  laneMinutesByUnit: Map<number, number>;
...
type RotationFamilyAccumulator = {
  rawMinutes: number;
  termBuckets: Map<number, RotationTermBucketAccumulator>;
};

type RotationFamilyTermBucketLoadComputation = {
  termRank: number | null;
  termLabel: string | null;
  termGroupId: string | null;
  termCount: number | null;
...
type RotationFamilyLoadComputation = {
  ...
  termGroupId: string | null;
  termCount: number | null;
  termBuckets: RotationFamilyTermBucketLoadComputation[];
  subjectCodes: string[];
  subjectIds: number[];
};
...
function toCanonicalRotationTermLabel(termLabel: string | null | undefined, termRank: number | null): string | null {
  const normalizedRank = Number.isInteger(termRank) && Number(termRank) > 0 ? Number(termRank) : null;
  if (normalizedRank) {
    return `Term ${normalizedRank}`;
  }

  const trimmedLabel = (termLabel ?? '').trim();
  if (!trimmedLabel) {
    return null;
  }

  const rankMatch = trimmedLabel.match(/(\d+)/);
  if (rankMatch) {
...
function resolveLoadRotationTermMetadata(subject: AssignmentLoadShape['subject']) {
  const rotationFamily = resolveLoadRotationFamily(subject);
  const metadata = resolveRotationTermMetadata({
  ...
  return {
    ...metadata,
    termLabel: toCanonicalRotationTermLabel(metadata.termLabel, metadata.termRank),
  };
}
...
function buildRotationConcurrentLaneId(rotationFamily: string, termRank: number | null, unit: number): string {
  return `family:${rotationFamily}:term:${normalizeRotationTermLaneKey(termRank)}:${unit}`;
}

function parseRotationConcurrentLaneKey(laneKey: string): { family: string; termKey: number } | null {
...
function estimateProjectedRotationFamilyPeakMinutes(
  ...
  for (const [key, laneMinutes] of lanes.entries()) {
    ...
    termTotals.set(
      laneDescriptor.termKey,
      (termTotals.get(laneDescriptor.termKey) ?? 0) + Math.max(0, Number(laneMinutes) || 0),
    );
  }
...
function resolveDominantRotationBucket(
  termBuckets: RotationFamilyTermBucketLoadComputation[],
): RotationFamilyTermBucketLoadComputation | null {
...
function computeTeachingLoadMinuteComputation(
  ...
  let rawMinutes = 0;
  const nonRotationLaneMinutes = new Map<string, number>();
    ...
    for (const unit of units) {
      ...
      if (rotationFamily) {
        const familyEntry = rotationFamilyStats.get(rotationFamily) ?? {
          rawMinutes: 0,
          termBuckets: new Map<number, RotationTermBucketAccumulator>(),
        };
        ...
        const termBucket = familyEntry.termBuckets.get(termKey) ?? {
          ...
          laneMinutesByUnit: new Map<number, number>(),
        ...
        const laneKey = `subject:${subjectLaneIdentity}:${unit}`;
        const currentLaneMinutes = nonRotationLaneMinutes.get(laneKey) ?? 0;
        if (perUnitMinutes > currentLaneMinutes) {
          nonRotationLaneMinutes.set(laneKey, perUnitMinutes);
        }
      }
    }
  }
  ...
  const rotationFamilies = Array.from(rotationFamilyStats.entries())
    .map(([family, value]) => {
      const termBuckets = Array.from(value.termBuckets.values())
        .map<RotationFamilyTermBucketLoadComputation>((bucket) => ({
      ...
      const credited = dominantTermBucket?.creditedMinutes ?? 0;
  ...
  return {
    rawMinutes,
    creditedMinutes:
      Array.from(nonRotationLaneMinutes.values()).reduce((sum, value) => sum + value, 0)
      + rotationFamilies.reduce((sum, family) => sum + family.creditedMinutes, 0),
    rotationFamilies,
  };
}
...
type RotationTermBucketBreakdownAccumulator = {
  termRank: number | null;
  termLabel: string | null;
  termGroupId: string | null;
  termCount: number | null;
...
type RotationFamilyBreakdownAccumulator = {
  rawMinutesPerWeek: number;
  termBuckets: Map<number, RotationTermBucketBreakdownAccumulator>;
};

export function buildRotationTermBreakdown(
  assignments: AssignmentRotationBreakdownInput[],
): RotationFamilyTermBreakdown[] {
  const familyBuckets = new Map<string, RotationFamilyBreakdownAccumulator>();
  ...
  for (const assignment of assignments) {
    ...
    const sections = assignment.sections
      .filter((section) => Number.isInteger(section.id) && section.id > 0)
      .sort((left, right) => left.id - right.id);
    if (sections.length === 0) {
      continue;
    }

    const termMetadata = resolveLoadRotationTermMetadata(assignment.subject);
    const termKey = normalizeRotationTermLaneKey(termMetadata.termRank);

    const familyEntry = familyBuckets.get(rotationFamily) ?? {
      rawMinutesPerWeek: 0,
      termBuckets: new Map<number, RotationTermBucketBreakdownAccumulator>(),
    };

    const termBucket = familyEntry.termBuckets.get(termKey) ?? {
      termRank: termMetadata.termRank,
      termLabel: termMetadata.termLabel,
      termGroupId: termMetadata.termGroupId,
      termCount: termMetadata.termCount,
      rawMinutesPerWeek: 0,
    ...
    if (termBucket.termRank === null && termMetadata.termRank !== null) {
      termBucket.termRank = termMetadata.termRank;
    }
    if (termBucket.termLabel === null && termMetadata.termLabel) {
      termBucket.termLabel = termMetadata.termLabel;
    }
    if (termBucket.termGroupId === null && termMetadata.termGroupId) {
      termBucket.termGroupId = termMetadata.termGroupId;
    }
    if (termBucket.termCount === null && termMetadata.termCount) {
      termBucket.termCount = termMetadata.termCount;
    }
    ...
    const normalizedSubjectId = Number.isInteger(assignment.subjectId) && assignment.subjectId > 0
      ? assignment.subjectId
      : Number.isInteger(assignment.subject.id) && Number(assignment.subject.id) > 0
        ? Number(assignment.subject.id)
        : null;
    if (normalizedSubjectId) {
      termBucket.subjectIds.add(normalizedSubjectId);
    }
    ...
    for (const section of sections) {
      termBucket.rawMinutesPerWeek += minutes;
      familyEntry.rawMinutesPerWeek += minutes;

      const currentLaneMinutes = termBucket.laneMinutesBySection.get(section.id) ?? 0;
      if (minutes > currentLaneMinutes) {
        termBucket.laneMinutesBySection.set(section.id, minutes);
      }
      const sectionName = section.name.trim();
      if (sectionName.length > 0) {
        termBucket.sectionNamesById.set(section.id, sectionName);
      }
    }

    familyEntry.termBuckets.set(termKey, termBucket);
    familyBuckets.set(rotationFamily, familyEntry);
  }

  return Array.from(familyBuckets.entries())
    .map(([family, familyEntry]) => {
      ...
      const peakTermBucket = [...termBuckets]
        .sort((left, right) => {
          if (right.creditedMinutesPerWeek !== left.creditedMinutesPerWeek) {
            return right.creditedMinutesPerWeek - left.creditedMinutesPerWeek;
          }
      ...
      for (const bucket of termBuckets) {
        bucket.isPeakTerm = peakTermBucket !== null
          && bucket.termRank === peakTermBucket.termRank
          && bucket.creditedMinutesPerWeek === peakTermBucket.creditedMinutesPerWeek;
      }

      return {
        family,
        rawMinutesPerWeek: familyEntry.rawMinutesPerWeek,
        peakTermMinutesPerWeek: peakTermBucket?.creditedMinutesPerWeek ?? 0,
        peakTermRank: peakTermBucket?.termRank ?? null,
        peakTermLabel: peakTermBucket?.termLabel ?? null,
        termGroupId: peakTermBucket?.termGroupId ?? null,
        termCount: peakTermBucket?.termCount ?? null,
        termBuckets,
      } satisfies RotationFamilyTermBreakdown;
    })
    .sort((left, right) => {
      if (right.peakTermMinutesPerWeek !== left.peakTermMinutesPerWeek) {
        return right.peakTermMinutesPerWeek - left.peakTermMinutesPerWeek;
      }
      return left.family.localeCompare(right.family);
    });
}
...
function normalizeAssignmentSummaryListOptions(options?: AssignmentSummaryListOptions): Required<Omit<AssignmentSummaryListOptions, 'department'>> & { department: string | null } {
  return {
...
function compareAssignmentSummaryName(left: AssignmentSummaryListRow, right: AssignmentSummaryListRow): number {
...
function buildAssignmentSummaryPage<T extends AssignmentSummaryListRow>(
  ...
  const departments = Array.from(
    new Set(rows.map((row) => row.department).filter((value): value is string => typeof value === 'string' && value.trim().length > 0)),
...
export async function getSectionAssignedClassesIndex(
  schoolId: number,
  schoolYearId: number,
  authToken?: string,
  options?: SectionAssignedClassesQueryOptions,
): Promise<SectionAssignedClassesIndexResult> {
  ...
  const expectedSubjectIdsBySection = new Map<number, Set<number>>();
  ...
  const classesBySection = new Map<number, SectionAssignedClassRow[]>();
  const staleOwnershipBySection = new Map<number, SectionStaleOwnershipDiagnosticRow[]>();
  const assignedSubjectIdsBySection = new Map<number, Set<number>>();
  for (const section of sectionScope) {
    classesBySection.set(section.id, []);
    staleOwnershipBySection.set(section.id, []);
    assignedSubjectIdsBySection.set(section.id, new Set<number>());
  }
  ...
  const sections: SectionAssignedClassesResult[] = sectionScope.map((section) => {
    const sectionClasses = classesBySection.get(section.id) ?? [];
    const sortedClasses = sectionClasses.sort((left, right) => {
      if (left.subjectCode !== right.subjectCode) return left.subjectCode.localeCompare(right.subjectCode);
      return left.facultyName.localeCompare(right.facultyName);
    });

    const expectedSubjectIds = expectedSubjectIdsBySection.get(section.id) ?? new Set<number>();
    const assignedSubjectIds = assignedSubjectIdsBySection.get(section.id) ?? new Set<number>();
    const unassignedExpectedSubjects = Array.from(expectedSubjectIds)
      ...
      .map<SectionUnassignedExpectedClassRow>((subject) => ({
    ...
    const totals: SectionAssignedClassesTotals = {
    ...
    const sectionResult: SectionAssignedClassesResult = {
      sectionId: section.id,
      sectionName: section.name,
      gradeLevel: section.gradeLevel,
      programType: section.programType,
...
export async function getSectionAssignedClasses(
  sectionId: number,
  schoolYearId: number,
  authToken?: string,
...
function normalizeRecoverySubjectCodes(subjectCodes?: string[]): string[] {
...
function getRotationGateVerdict(
  family: 'SCIENCE' | 'TLE_ROTATION',
  facultyRows: Array<{ isPlaceholder: boolean; rotationFamilyLoadDetails?: RotationFamilyLoadDetail[] }>,
): RotationGateVerdict {
...
export async function previewOrApplyRealFacultyRecovery(
  input: RealFacultyRecoveryInput,
): Promise<RealFacultyRecoveryResult> {
  const apply = input.apply === true;
  ...
  const creditedMinutesByFaculty = new Map<number, number>();
  ...
  for (const row of ownershipRows) {
    ...
    const lanes = lanesByFaculty.get(row.facultyId) ?? new Map<string, number>();
    const currentLaneMinutes = lanes.get(laneKey) ?? 0;
    if (perUnitMinutes > currentLaneMinutes) {
  ...
  const plannedMoves: RealFacultyRecoveryMove[] = [];
  const blockers: RealFacultyRecoveryBlocker[] = [];
  ...
  const rotationFamilyBySubjectId = new Map<number, string | null>();
  const rotationFamilyCountByFamilyAndFaculty = new Map<string, Map<number, number>>();
  ...
  for (const row of targetRows) {
    ...
    if (family) {
      incrementNestedCount(rotationFamilyCountByFamilyAndFaculty as Map<number | string, Map<number, number>>, family, row.facultyId, 1);
    }
  }
  ...
  pendingPairs.sort((left, right) => {
    if (left.subjectId !== right.subjectId) return left.subjectId - right.subjectId;
  ...
  for (const pair of pendingPairs) {
    ...
    const candidates = candidateFacultyRows
      ...
      .sort((left, right) => {
        ...
        const leftLanes = lanesByFaculty.get(left.id) ?? new Map<string, number>();
        const rightLanes = lanesByFaculty.get(right.id) ?? new Map<string, number>();
    ...
    for (const candidate of candidates) {
      ...
      const lanes = lanesByFaculty.get(candidate.id) ?? new Map<string, number>();
      const currentLaneMinutes = lanes.get(laneKey) ?? 0;
    ...
    const selectedLanes = lanesByFaculty.get(selectedCandidate.id) ?? new Map<string, number>();
    ...
    creditedMinutesByFaculty.set(
      selectedCandidate.id,
      (creditedMinutesByFaculty.get(selectedCandidate.id) ?? 0) + selectedDeltaMinutes,
    );
    ...
    if (rotationFamily) {
      incrementNestedCount(rotationFamilyCountByFamilyAndFaculty as Map<number | string, Map<number, number>>, rotationFamily, selectedCandidate.id, 1);
    }
    ...
    if (pair.fromFacultyId > 0) {
      ...
      if (rotationFamily) {
        incrementNestedCount(rotationFamilyCountByFamilyAndFaculty as Map<number | string, Map<number, number>>, rotationFamily, pair.fromFacultyId, -1);
      }
    }
  }
  ...
  const subjectDeltas: RealFacultyRecoverySubjectDelta[] = targetSubjectCodes.map((code) => {
  ...
  const addIntegrityBlockerSamples = (
    samples: TeachingLoadIntegrityDiagnosticRow[] | undefined,
    reasonPrefix: string,
  ...
  if ((integrityDiagnostics.currentYearMissingOwnershipPairs ?? 0) > 0) {
    addIntegrityBlockerSamples(
      integrityDiagnostics.missingOwnershipSamples,
      'Missing ownership pairs remain.',
  ...
  if ((integrityDiagnostics.currentYearOwnershipWithoutMatchingScopePairs ?? 0) > 0) {
    addIntegrityBlockerSamples(
      integrityDiagnostics.ownershipWithoutScopeSamples,
      'Ownership exists outside stored assignment scope.',
  ...
  if ((integrityDiagnostics.currentYearOutOfSubjectScopePairs ?? 0) > 0) {
    addIntegrityBlockerSamples(
      integrityDiagnostics.outOfSubjectScopeSamples,
      'Subject scope leakage detected.',
...
function buildSpecialProgramRedistributionInsights(
  ...
  return subjects.map((subject) => {
    ...
    const qualifiedCandidates = [...facultyById.values()]
      ...
      .filter((member) => {
        const departmentQualified = matchesSubjectOwnershipDepartment(
          member.department,
          subject.code,
          subject.name,
          subject.ownerDepartment,
        ...
        return Array.from(requiredSpecializationCodes).some((requiredCode) =>
          hasApprovedCapabilityOverride(overridesByFacultyId, member.id, subject.code, requiredCode),
        );
      });
    ...
    const candidateSignals: SpecialProgramCandidateSignal[] = qualifiedCandidates
      .map((candidate) => {
        ...
        for (const sectionId of subject.relevantSectionIds) {
          ...
          const overrideSupports = hasApprovedCapabilityOverride(
        ...
        return {
          ...
          canCoverConstrainedSection:
            specializationExactMatchSectionCount > 0
            || Array.from(requiredSpecializationCodes).some((requiredCode) =>
...
export async function listTeachingLoadCapabilityOverrides(
  schoolId: number,
  schoolYearId: number,
...
export async function previewOrApplySpecialProgramRedistribution(
  ...
  const overridesByFacultyId = new Map<number, TeachingLoadCapabilityOverride[]>();
...
function normalizeSpecializationCode(value: string | null | undefined): string | null {
...
function normalizeSpecializationLabel(value: string | null | undefined): string | null {
const trimmed = (value ?? '').trim();
return trimmed.length > 0 ? trimmed : null;
}

function normalizeOverrideSubjectCode(value: string | null | undefined): string | null {
  const normalized = (value ?? '').trim().toUpperCase();
  return normalized.length > 0 ? normalized : null;
}

function getTeachingLoadCapabilityOverridesFromConfig(
  constraintConfig: Prisma.JsonValue | null | undefined,
): TeachingLoadCapabilityOverride[] {
  if (!constraintConfig || typeof constraintConfig !== 'object' || Array.isArray(constraintConfig)) {
    return [];
  }

  const rawOverrides = (constraintConfig as Record<string, unknown>).teachingLoadCapabilityOverrides;
  if (!Array.isArray(rawOverrides)) {
    return [];
  }

  const parsed: TeachingLoadCapabilityOverride[] = [];
  for (const entry of rawOverrides) {
    if (!entry || typeof entry !== 'object' || Array.isArray(entry)) continue;
    const row = entry as Record<string, unknown>;
    const facultyId = Number(row.facultyId);
    if (!Number.isInteger(facultyId) || facultyId <= 0) continue;

    const subjectCode = normalizeOverrideSubjectCode(typeof row.subjectCode === 'string' ? row.subjectCode : null);
    const specializationCode = normalizeSpecializationCode(
      typeof row.specializationCode === 'string' ? row.specializationCode : null,
    );
    const specializationLabel = normalizeSpecializationLabel(
      typeof row.specializationLabel === 'string' ? row.specializationLabel : null,
    );
...
function buildConstraintConfigWithTeachingLoadOverrides(
  constraintConfig: Prisma.JsonValue | null | undefined,
  overrides: TeachingLoadCapabilityOverride[],
): Prisma.InputJsonValue {
  const base: Record<string, unknown> =
    constraintConfig && typeof constraintConfig === 'object' && !Array.isArray(constraintConfig)
      ? { ...(constraintConfig as Record<string, unknown>) }
      : {};
...
function capabilityOverrideMatches(
  override: TeachingLoadCapabilityOverride,
  subjectCode: string,
...
function canonicalizeAllowedSpecialization(
...
function buildAssignmentLaneImpactByPair(assignments: Array<{
  ...
  const nonRotationLaneCredits = new Map<string, number>();
  const rotationFamilyPeakMinutes = new Map<string, number>();
  const rotationFamilyTermTotals = new Map<string, Map<number, number>>();
  const rotationFamilyTermLaneCredits = new Map<string, Map<number, Map<number, number>>>();
  ...
  for (const assignment of orderedAssignments) {
    ...
    for (const sectionId of [...assignment.sectionIds].sort((left, right) => left - right)) {
      let laneId = `subject:${assignment.subjectId}:${sectionId}`;
      let concurrentDeltaMinutesPerWeek = 0;

      if (family) {
        const termKey = normalizeRotationTermLaneKey(termMetadata.termRank);
        laneId = buildRotationConcurrentLaneId(family, termMetadata.termRank, sectionId);

        const termLanes = rotationFamilyTermLaneCredits.get(family) ?? new Map<number, Map<number, number>>();
        const sectionLaneCredits = termLanes.get(termKey) ?? new Map<number, number>();
        const currentLaneMinutes = sectionLaneCredits.get(sectionId) ?? 0;
        const laneIncrease = Math.max(0, minutes - currentLaneMinutes);

        const termTotals = rotationFamilyTermTotals.get(family) ?? new Map<number, number>();
        const currentTermTotal = termTotals.get(termKey) ?? 0;
        const currentPeakMinutes = rotationFamilyPeakMinutes.get(family) ?? 0;

        if (laneIncrease > 0) {
          sectionLaneCredits.set(sectionId, minutes);
          termLanes.set(termKey, sectionLaneCredits);
          rotationFamilyTermLaneCredits.set(family, termLanes);

          const updatedTermTotal = currentTermTotal + laneIncrease;
          termTotals.set(termKey, updatedTermTotal);
          rotationFamilyTermTotals.set(family, termTotals);

          const updatedPeakMinutes = Math.max(currentPeakMinutes, updatedTermTotal);
          rotationFamilyPeakMinutes.set(family, updatedPeakMinutes);
          concurrentDeltaMinutesPerWeek = Math.max(0, updatedPeakMinutes - currentPeakMinutes);
        }
      } else {
        const creditedSoFar = nonRotationLaneCredits.get(laneId) ?? 0;
        concurrentDeltaMinutesPerWeek = Math.max(0, minutes - creditedSoFar);
        if (minutes > creditedSoFar) {
          nonRotationLaneCredits.set(laneId, minutes);
        }
      }
...
function resolveOwnedCurrentYearSectionScope(
  storedCurrentYearSectionIds: number[],
  ownedCurrentYearSectionIdsRaw: number[],
  relevantSectionIds: number[],
): AssignmentOwnedSectionScopeResolution {
...
export function __testResolveOwnedCurrentYearSectionScope(
  storedCurrentYearSectionIds: number[],
  ownedCurrentYearSectionIdsRaw: number[],
  relevantSectionIds: number[],
): AssignmentOwnedSectionScopeResolution {
  return resolveOwnedCurrentYearSectionScope(
    storedCurrentYearSectionIds,
...
for (const row of ownershipRows) {
  ...
  const specializationMap = specializationByFacultySubjectId.get(row.facultySubjectId) ?? new Map<number, AssignmentSpecializationIdentity>();
...
const schedulableAssignments = assignments.filter(
  (assignment) => assignment.subject.isActive && assignment.subject.code !== HG_SUBJECT_CODE,
);
const laneImpactByPair = buildAssignmentLaneImpactByPair(
  schedulableAssignments.map((assignment) => ({
    subjectId: assignment.subjectId,
...
} = resolveOwnedCurrentYearSectionScope(
  storedCurrentYearSectionIds,
  ownedCurrentYearSectionIdsRaw,
  relevantSectionIds,
);
...
rotationTermBreakdown: buildRotationTermBreakdown(assignmentResponses),
};
}
...
export async function getAssignmentSummary(
  ...
  const facultySubjectOwnerById = new Map<number, number>();
  ...
  const outOfSubjectScopeSamples: TeachingLoadIntegrityDiagnosticRow[] = [];
  ...
  const facultySummary = faculty
    ...
    const schedulableAssignments = member.facultySubjects.filter(
      (assignment) => assignment.subject.isActive && assignment.subject.code !== HG_SUBJECT_CODE,
    );
    const laneImpactByPair = buildAssignmentLaneImpactByPair(
    schedulableAssignments.map((assignment) => ({
    ...
    return {
      ...
      rotationTermBreakdown: buildRotationTermBreakdown(assignments),
      gradeTeachingHours,
      advisoryHours,
      ancillaryHours,
      policyCreditedHours,
...
export async function previewOrApplyTeachingLoadTruthReconcile(
  ...
  const updates: Array<{
    ...
    nextCurrentYearSectionCount: number;
    outOfSubjectScopeSectionCount: number;
    outOfSubjectScopeOwnedSectionIds: number[];
...
export async function getFacultyAssignmentIdentitySummary(
  facultyId: number,
  schoolYearId: number,
  authToken?: string,
): Promise<Array<{
